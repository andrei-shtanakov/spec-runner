...
# Значение НЕ обязательно для совпадения: строка ровно
# `**Negative-control:**` иначе не совпадала ни с чем и падала сквозь
# разбор, делая задачу неотличимой от задачи БЕЗ маркера, — а `validate`
# и отказ точки 1 утверждали «маркера нет» про файл, который его
# содержит. Диагноз переворачивался невидимым пробелом на конце строки.
# Годность значения судит `_parse_negative_control`, и пустое он называет
# по имени: канал ошибки существует ровно затем, чтобы «маркера нет» и
# «маркер негоден» оставались разными дефектами.
