...
| `error_stage` | TEXT | experimental | Added v2.3.0. Sub-stage when failure occurred (one of `sync_deps`, `branch`, `verify`, `exec`, `parse`, `tests`, `lint`, `commit`, `merge`, `review`); nullable. `exec` replaced `codex` in v2.13 — rows written by ≤2.12 may still carry `codex`. `verify` (v2.35.0, #367 BEH-30) is the live verify-first run, before any paid call — distinct from `tests`, which judges the candidate tree after the implementation pass |
