...
#### BEH-48: Подкоманда без платного вызова не завершается успешно, пока её checkpoint не acknowledged, а `restore` не решает по store, пока не доставлены унаследованные обязательства
...
- **And** `restore` ждёт доставки ДО чтения store, а не после: в проекте,
  где предыдущий прогон убит между commit-ом своей mutation и доставкой её
  checkpoint-а (обязательство лежит в локальной DB), выполняется `restore
  <run_id> --into <dir> --experimental` с задерживающим двойником. В общем
  журнале ack унаследованного checkpoint-а стоит **раньше первого
  обращения `restore` к store** — раньше `list` индекса workstream-а и
  раньше чтения digests последнего acknowledged checkpoint-а, которым
  порядок проверок начинается. Любое чтение раньше ack — красный тест, и
  не по букве: устаревший store искажает и вердикт проверки (6), и сам
  выбор материала — «последним acknowledged» оказался бы не тот
  checkpoint.
- **And** с отклоняющим ack двойником `restore` завершается exit 2 до
  применения snapshot-а: `--into` остаётся пустым, ни одного `Popen`, а
  stderr называет недоставленный `sequence` и `checkpoint_id`. Применённый
  snapshot здесь — красный тест: молчаливая потеря чужой правки есть ровно
  тот исход, ради которого забор и поставлен.
- **And** выбор задачи забором не накрыт **в ветке (1) Q-12**, и это
  предъявлено на ней: `run --all` в том же проекте — маркер
  `continuation_index: local`, то есть индексом объявляет себя локальная
  DB — с тем же унаследованным обязательством выбирает задачу без
  ожидания; до своего первого call-start (сайт (а)) ack не требуется.
  Ожидание перед выбором задачи здесь — красный тест, и причина
  проверяемая: недоставленным в этой ветке может быть только **закрытие**
  строки ledger-а, поэтому устаревший store отвечает «всё ещё open» и
  прогон задачу пропускает — ошибка fail-closed. Ждать ради того, чтобы
  разрешить себе больше работы, правило не требует.
  Ветка (2) (маркера нет либо он `restored`) судит по индексу
  workstream-а и под область (в) подпадает — но забор в ней пуст по
  построению: DB свежая, удалённая или приехавшая из snapshot-а,
  обязательств в ней нет. Сценарий это и наблюдает: после `reset` тот же
  `run --all` идёт веткой (2), очередь publisher-а на старте пуста, и
  ожидания снова нет — но уже потому, что ждать нечего, а не потому, что
...
- **And** горячий путь `run` не получает ни одного нового синхронного
  ...
  invocation-а по-прежнему столько, сколько сайтов правила Q-05 он
  проходит, и число mutation подкоманды его не увеличивает.
