...
- **And** `restore` более раннего `run_id` workstream-а, у которого есть
  ...
  **исполнением**, а не чтением строки: тест берёт напечатанный `run_id` и
  ...
  snapshot. Отказ на напечатанном выходе — красный тест; отдельно предъявлен
  случай, где кандидата нет (A → B `plan` с checkpoint-ом → C
  `budget authorize`, убитый без closure): выход не печатается вовсе, отказ
  называет `run_id` блокирующей C, а печать B здесь — красный тест, потому
  что `restore <B>` упрётся в ту же C. Предъявлено как минимум на `review-pr`
...
- **And** более поздний **холостой** прогон из той же блокирующей половины
  ...
  executor lock-ом уже после run-start. Оба **закрылись штатно** — closure
  есть и неподтверждённого checkpoint-а она не называет, — оба лежат в
  индексе позже A, ни один не опубликовал checkpoint: `restore <run_id-A>`
  применяет snapshot. Отказ здесь — красный тест, и не только по букве: он
  оставляет оператора без единого пути, потому что восстановить названный
  выходом холостой прогон нельзя — digests берутся из последнего
  acknowledged checkpoint-а, которого у него нет.
- **And** пустота без closure зелёным не считается: более поздний
  `budget authorize`, убитый `kill -9` в окне между записью в DB и
  доставкой publisher-ом, оставляет run-start и строку индекса, но ни
  closure, ни ключей `runs/<run_id>/checkpoints/…`. `restore <run_id-A>`
  отказывает `needs-human`, называя `run_id` этого прогона. Применённый
  здесь snapshot A — красный тест: отсутствие checkpoint-ов у прогона без
  closure означает «неизвестно, что он оставил», а не «ничего», и молчаливая
  потеря `budget_authorizations` — ровно то, что запрещает FR-05. Тот же
  знак — на прогоне, чья closure называет неподтверждённый checkpoint
  (BEH-31): она честная, но пустоту не доказывает.
