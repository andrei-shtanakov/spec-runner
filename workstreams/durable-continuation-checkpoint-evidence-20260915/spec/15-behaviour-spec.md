...
- **And** `restore` ждёт доставки ДО чтения store, а не после: в проекте,
  ...
  журнале ack унаследованного checkpoint-а стоит **раньше** первого `list`
  индекса workstream-а. Порядок «`list` раньше ack» — красный тест, и не
  по букве: именно он даёт вердикт, посчитанный по store, который о той
  mutation не знает.
