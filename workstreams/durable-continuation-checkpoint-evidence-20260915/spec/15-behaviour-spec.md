...
- **And** чего этот сценарий про забор на чтение (сайт (в) правила Q-05)
  **не** предъявляет и почему: вход для него — прогон, убитый между
  commit-ом своей mutation и доставкой её checkpoint-а, чьё обязательство
  подхватывает следующий процесс. Построить такой вход бандлом нечем:
  obligation-строки в транзакции mutation и их подъём в очередь publisher-а
  при первом за процесс открытии DB — механизм spec-runner#528, а очередь
  publisher-а процессная, и у нового процесса `restore` она пуста по
  построению. Знак «ack раньше первого обращения к store» на пустой очереди
  выполнялся бы вхолостую — красил бы неизвестное зелёным, — а знак «с
  отклоняющим ack двойником `restore` выходит 2» недостижим: отклонять
  нечего. Поэтому правило записано (Q-05, § 7.2), а его наблюдаемое
  приезжает вместе с механизмом — в #528, который владеет и тем, и другим.
  Здесь это названо, а не умолчано: сценарий с пустой очередью, выданный за
  проверку забора, и есть тот способ покрасить неизвестное зелёным, против
  которого написан весь бандл.
- **And** горячий путь `run` не получает ни одного нового синхронного
  ...
  проходит (для `run` это (а) и (б)), и число mutation подкоманды его не
  увеличивает.
