...
- **And** первый `run` в восстановленном каталоге спрашивает заново, а не
  ...
  подразумевается: ни C, ни D не несут acknowledged checkpoint-а — оба
  убиты на границах Given (после ack call-start либо после возврата
  `Popen`), то есть до первой continuation-relevant mutation своего
  прогона, — поэтому второе условие ключа шага 5 проверки (6) на них не
  выполняется. Дверь здесь ни при чём: она закрывает **call**, не прогон.
