...
- **And** обязательство, для которого локальных байтов не существует, не
  ...
  checkpoint текущего состояния, в котором та mutation есть, и прогон
  доходит до своей работы. Отказ здесь — красный тест: обязательство
...
- **And** следующий `run` в новом каталоге **запускается**, а не отказывает
  инструментом: восстановленная DB не требует доставить checkpoint, байтов
  которого в этом каталоге нет. Знак — прогон `run --task <id>` после
  restore доходит до своей работы (двойник store получает его call-start), а
  не exit 2 с недоставленным `sequence`. Это про унаследованное состояние, а
  не про файлы: snapshot снимается раньше ack своего же checkpoint-а, и
  обязательство опубликовать его в snapshot попадает; восстанавливаются же
  из **acknowledged** checkpoint-а, то есть всё, на что такое обязательство
  указывает, уже доставлено. Отказ здесь — красный тест: восстановленный
  каталог не запускается ровно в том сценарии, ради которого restore
  существует.
