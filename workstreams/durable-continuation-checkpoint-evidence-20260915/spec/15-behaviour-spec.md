...
- **And** отказ гварда на `_spawn` переживает `except Exception` в
  `execute_task`. Предъявлено там, где гвард не подменён двойником, — в
  `tests/test_harness_guards.py`: тест доходит до GREEN с настоящим именем
  `claude`, и наблюдаемое — падение с вердиктом гварда, а не обычный
  неудачный attempt с `return False`. Зелёный тест, ожидавший неуспеха задачи, — красный
  результат этого And: он означает, что отказ проглочен и гвард больше
  ничего не доказывает. Пояс `PaidBinaryReached` этот случай не
  подстраховывает — процесс не создавался.
- **And** call-start сайта `doctor` несёт provenance `doctor` и **пустой**
  `task_id`: `TASK-001` канонной пробы doctor-а не появляется ни в одной
  записи, адресуемой задачам проекта. Предъявлено на конфигурации, где эти
  два ответа расходятся: проба падает между ack call-start и записью
  результата, после чего в проекте с собственной задачей `TASK-001`
  выполняется `run --all` после `spec-runner reset` (ветка (2) процедуры
  открытых calls). Реальная `TASK-001` выбирается и исполняется, а open call
  пробы предъявлен отдельно — с пустым `task_id` и provenance `doctor`.
  Заблокированная реальная `TASK-001` — красный тест.
...
- **And** `restore` более раннего `run_id` workstream-а, у которого есть
  более поздний **закрытый** прогон, изменивший continuation-state, —
  обычный `run`/`retry`/`watch`, `budget authorize` или
  ...
  workstream-а.
- **And** более поздний прогон, continuation-state **не** менявший, этого
  отказа не даёт, и решает здесь свойство прогона, а не его имя.
  Предъявлено на трёх, каждый из которых оставляет в индексе workstream-а
  свою строку: дверь `evidence close-call`, закрывшая X двумя And выше;
  `spec-runner doctor`, прошедший между A и восстановлением; и **сам `restore`**,
  выполненный дважды подряд — его собственный run-start ложится в индекс
  раньше, чем эта проверка читает индекс. Во всех трёх snapshot A
  применяется. Отказ на любом из трёх — красный тест: он означает, что
  реализация ключуется на перечне имён подкоманд, а не на том, оставил ли
  прогон изменения, которых в snapshot-е нет.
