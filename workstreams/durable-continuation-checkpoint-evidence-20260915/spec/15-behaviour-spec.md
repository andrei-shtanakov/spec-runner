...
- **And** выбор задачи забором не накрыт **в ветке (1) Q-12**, и это
  ...
  workstream-а, но обязательств у процесса в ней нет по построению — DB
  свежая, удалённая или приехавшая из snapshot-а. Сценарий это и
  наблюдает: после `reset` тот же `run --all` идёт веткой (2), очередь
  publisher-а на старте пуста, ожидания снова нет — и причина другая, чем
  в ветке (1): там правило его не требует, здесь требовать нечего.
