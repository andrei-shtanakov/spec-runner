...
сообщает об успехе до ack своего mutation-checkpoint-а; недоставленная
...
- **And** mutation при этом не откатывается и не теряется: строка
  ...
  она была закоммичена раньше. Наблюдаемое здесь — сама mutation и, ниже,
  её поздняя доставка; носитель обязательства опубликовать (таблица, её
...
- **And** горячий путь `run` не получает **ни одного** нового синхронного
  ...
  следующим шагом. Цена третьей точки drain у подкоманд без платного вызова
  меряется бенчмарком (BEH-41), а не этим сценарием (RK-01).
