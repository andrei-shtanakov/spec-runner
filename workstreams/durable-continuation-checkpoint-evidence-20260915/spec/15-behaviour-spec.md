...
- **And** чего этот сценарий **не** предъявляет и почему: поведения
  `restore`, который дожидался бы доставки прежде, чем судить по store.
  Вход для такого знака — прогон, убитый между записью своей mutation и
  доставкой её checkpoint-а, чью недоставленную правку следующий процесс
  обязан увидеть; ни один сценарий этого бандла такого входа не
  производит, потому что самой этой правки как durable-обязательства
  бандл не вводит (spec-runner#528, тот же остаток, что назван абзацем
  выше). На входе, которого нет, знак «дождался прежде, чем прочитал»
  выполнялся бы сам собой, а «отказал, не дождавшись» был бы недостижим —
  и первое есть ровно тот способ покрасить неизвестное зелёным, против
  которого написан весь бандл. Поэтому знак назван здесь отсутствующим, а
  предъявляется он вместе с механизмом.
- **And** горячий путь `run` не получает ни одного нового синхронного
  ...
  invocation-а по-прежнему столько, сколько раз он совершает действия
  правила Q-05 — у `run` это по одному перед каждым платным вызовом плюс
  одно на выходе, — и число mutation подкоманды его не увеличивает.
