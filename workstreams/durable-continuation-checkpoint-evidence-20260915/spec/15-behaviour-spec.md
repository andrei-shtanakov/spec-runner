...
сообщает об успехе до ack своего mutation-checkpoint-а), и BEH-48 предъявляет
именно его — наблюдаемый порядок двух событий, а не устройство доставки,
очередь, таблицу обязательств или число ожиданий; где живёт единый seam
...
- **And** горячий путь `run` ожидания на каждую mutation не получает: в
  прогоне `run --task` с пустым на старте outbox-ом число синхронных
  ожиданий ack равно числу точек drain перед платными вызовами плюс одна
  перед closure, и ни одна mutation задачи не ждёт ack отдельно от них.
  Ожидание на mutation, выросшее в `run`, — красный тест (RK-01).
- **And** одно ожидание в `run` всё же наблюдаемо, и именно то, ради
  которого outbox существует: когда на старте в нём есть незакрытая строка
  (предыдущий прогон убит в окне), `run` доставляет её прежде выбора задачи
  — это ремонт, а не цена каждой mutation, и он наблюдается ровно один раз
  на такую строку: второй `run` подряд уже не ждёт.
