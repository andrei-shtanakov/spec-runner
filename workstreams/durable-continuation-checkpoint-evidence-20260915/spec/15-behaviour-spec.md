...
- **And** `restore` более раннего `run_id` workstream-а, у которого есть
  ...
  continuation-relevant по § 3 требований), `plan` (дописывает задачи в
  `tasks.md`) и `doctor` (его проба гоняет `execute_task`, пишущий
  `attempts`) — тоже отказывает `needs-human`: эти изменения в snapshot A не
  попали, и отказ называет последний `run_id` workstream-а. Эти три названы
  поимённо, потому что «платит, но задач не выбирает» — не то же самое, что
  «ничего не оставил»: снятый на `review-pr` отказ означает потерянные
  `pr_review_comments` и переплату за те же комментарии.
- **And** более поздний прогон, ничего такого **не** оставивший, этого
  ...
  шаг close пишет `agent_calls` и публикует checkpoint — ни то, ни другое в
  перечень § 3 требований не входит), и **сам `restore`**, выполненный
  дважды подряд: его собственный run-start ложится в индекс раньше, чем эта
  проверка читает индекс. В обоих snapshot A применяется. Отказ на любом из
  двух — красный тест: он означает, что реализация ключуется на перечне имён
  подкоманд или на факте checkpoint-а, а не на том, оставил ли прогон
  изменения, которых в snapshot-е нет.
