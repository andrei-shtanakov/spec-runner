...
- **And** `restore` более раннего `run_id` workstream-а, у которого есть
  ...
  `tdd abandon|repair|resume|release`, а также `review-pr` (его раунд пишет
  `pr_review_comments` — таблицу из перечня § 3 требований) и `plan`
  (дописывает задачи в `tasks.md`) — тоже отказывает `needs-human`: эти
  изменения в snapshot A не попали, и отказ называет последний `run_id`
  workstream-а. Эти два названы поимённо, потому что «платит, но задач не
  выбирает» — не то же самое, что «ничего не добавил»: снятый на `review-pr`
  отказ означает потерянные `pr_review_comments` и переплату за те же
  комментарии.
- **And** более поздний прогон, ничего в namespace **не** добавивший, этого
  ...
  свою строку: дверь `evidence close-call`, закрывшая X двумя And выше — её
  запись есть шаг close уже существующего вызова, пометка «закрыт», и это
  так независимо от того, в какой ledger-таблице вызов жил (`agent_calls`
  или `pr_agent_calls`): конфигурация с pr-вызовом даёт тот же неотказ, и
  её отказ — такой же красный тест; `evidence purge`, отработавший между A и
  восстановлением (он open call не закрывает вовсе, а его
  `deletions/<ts>.json` — аудит собственного удаления, не рабочий материал);
  и **сам `restore`**, выполненный второй раз, в новый пустой `--into`
  (повторный запуск в уже занятый каталог отказывает раньше, на проверке
  каталога, и шага 5 не наблюдает): его собственный run-start ложится в
  индекс раньше, чем эта проверка читает индекс. Во всех трёх snapshot A
  применяется. Отказ на любом из трёх — красный тест: он означает, что
  реализация ключуется на перечне имён подкоманд или на факте checkpoint-а,
  а не на том, добавил ли прогон в namespace то, чего в snapshot-е нет.
