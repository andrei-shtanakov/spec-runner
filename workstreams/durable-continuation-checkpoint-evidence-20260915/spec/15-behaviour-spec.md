...
- **And** более поздний прогон, ничего в namespace **не** добавивший, этого
  ...
  Предъявлено на двух, каждый из которых оставляет в индексе workstream-а
  ...
  её отказ — такой же красный тест; и **сам `restore`**, выполненный второй
  раз, в новый пустой `--into` (повторный запуск в уже занятый каталог
  отказывает раньше, на проверке каталога, и шага 5 не наблюдает): его
  собственный run-start ложится в индекс раньше, чем эта проверка читает
  индекс. В обоих snapshot A применяется. Отказ на любом из двух — красный
  тест: он означает, что реализация ключуется на перечне имён подкоманд или
  на факте checkpoint-а, а не на том, добавил ли прогон в namespace то,
  чего в snapshot-е нет.
