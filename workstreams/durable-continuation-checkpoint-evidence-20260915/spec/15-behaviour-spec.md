...
- **And** `restore` более раннего `run_id` workstream-а, у которого есть
  более поздний **закрытый** прогон, оставивший изменения, которых snapshot
  не несёт, — `run`/`retry`/`watch`, `budget authorize`,
  `tdd abandon|repair|resume|release`, а также `review-pr` (его `pr_*` —
  continuation-relevant по § 3 требований) и `plan` (дописывает задачи в
  `tasks.md`) — тоже отказывает `needs-human`: эти изменения в snapshot A не
  попали, и отказ называет последний `run_id` workstream-а. `review-pr` и
  `plan` названы здесь поимённо, потому что «платит, но задач не выбирает» —
  не то же самое, что «ничего не оставил»: снятый на них отказ означает
  потерянные `pr_review_comments` и переплату за те же комментарии.
- **And** более поздний прогон, ничего такого **не** оставивший, этого
