...
что значит «доступен» (Q-05) — с одной оговоркой: **два** факта порядка
требования фиксируют сами (FR-03: подкоманда без платного вызова не
завершается успешно до ack своего mutation-checkpoint-а; недоставленная
чужая правка уезжает раньше собственных записей прогона, то есть раньше его
платного вызова и раньше его успешного завершения), и BEH-48
предъявляет ровно их — порядок наблюдаемых событий, а не устройство
доставки, очередь, носитель обязательства или число ожиданий; где живёт единый seam
...
- **And** правка, доехавшая **поздней доставкой**, блокирует так же, как
  доставленная сразу: в том же namespace `budget authorize` убит в окне
  между commit-ом mutation и ack (BEH-48), затем в том же каталоге
  отработал любой прогон, доставивший его недоставленный checkpoint, — и
  `restore <run_id-A>` отказывает `needs-human` шагом 5, назвав прогон
  `budget authorize`. Применённый snapshot A здесь — красный тест: это и
  есть та правка, которая без outbox-а терялась молча (BEH-48), и **этот**
  And — её единственный знак на поверхности restore. Предикат шага 5 при
  этом не менялся: прогон блокирует по тем же двум условиям ключа, просто
  его acknowledged checkpoint появился позже.
...
- **And** дверь доставляет **чужой** недоставленный checkpoint, и именно
  ...
  ранее убитый `budget authorize` оставил свою правку недоставленной
  (BEH-48), оператор выполняет `close-call --reason …`,
  ...
  authorize`. Дверь, завершившаяся с чужой правкой всё ещё
  недоставленной, — красный тест: путь «дверь → restore» тогда применяет
  snapshot старше чужого решения молча. Своего сайта ожидания у двери при этом нет —
...
- **And** ни одного обязательства опубликовать проба тоже не оставляет:
  после `doctor` в каталоге нет недоставленных правок, и следующий прогон не
  публикует ни одного checkpoint-а, чей DB-snapshot был бы снимком
  scratch-каталога, — ни под `run_id` `doctor`-а, ни под своим. Такой
  checkpoint (его manifest несёт identity временного каталога) — красный
  тест, и знак предъявляется прогоном **после** `doctor`, а не только во
  время него.
...
- **And** mutation при этом не откатывается и не теряется: строки в DB есть,
  ...
  ниже, поздняя доставка; носитель обязательства опубликовать (таблица, её
  имя, схема, транзакционность) — решение design-стадии, и этот сценарий
  его не утверждает.
- **And** после `kill -9` в окне между commit-ом mutation и ack (двойник
  вешает ack, тест убивает процесс) недоставленный checkpoint всё-таки
  доезжает: следующий прогон в том же каталоге отправляет его **раньше
  своих** записей и раньше двух событий, после которых прогон читают вне
  машины, — раньше собственного платного вызова и раньше своего
  успешного завершения. `checkpoint_id` тот же, повторная доставка
  идемпотентна, второго checkpoint-а на ту же mutation двойник не получает.
- **And** это предъявлено на трёх прогонах, накрывающих оба события:
  `run --all` (чужая строка у двойника раньше первого `spawn`),
  **второй** `spec-runner budget authorize` (чужая строка раньше его
  собственного нулевого кода выхода — платного вызова у него нет вовсе) и
  `spec-runner plan --gated` (чужая строка раньше его `spawn` — то есть
  раньше траты). Доставки раньше **выбора** задачи сценарий не требует:
  решение внутри каталога недоставленный checkpoint не меняет, mutation уже
  в локальной DB, — и требование такого порядка было бы требованием
  четвёртой точки ожидания, которой дизайн не вводит. Свойство, доказанное только на `run`, этот And не
...
- **And** read-only команды не доставляют: `status`, `costs`, `validate`,
  `report`, `evidence <run_id>` в том же каталоге с той же недоставленной
  правкой не отправляют двойнику ничего (BEH-04, BEH-13 остаются
  истинными), и следующий прогон после них доставляет её так же, как если бы
  их не запускали.
- **And** обязательство, для которого локальных байтов не существует, не
  делает каталог незапускаемым: `kill -9` в окне между commit-ом mutation и
  созданием файлов checkpoint-а (двойник `sqlite3.Connection.backup` вешает
  вызов, тест убивает процесс) оставляет обязательство без байтов, и
  следующий прогон **не** отказывает инструментом — двойник store получает
  checkpoint текущего состояния, в котором та mutation есть, под `run_id`
  **этого** прогона (а не убитого) и с записью о закрытом обязательстве
  убитого; прогон доходит до своей работы. Дубль `sequence` под чужим
  `run_id` — красный тест. Отказ здесь — красный тест: обязательство
  снимается только доставкой, поэтому отказ повторялся бы у каждого
  следующего прогона и каталог перестал бы запускаться навсегда.
- **And** доехавшая поздней доставкой правка становится для шага 5 такой
  же, как доставленная сразу, — но предъявлено это там, где живёт сам шаг 5
  и команда `restore`: **BEH-09** (задача после DT-06). Здесь этот
  сценарий утверждает только доставку; утверждать отказ restore он не может
  — на поверхности своей задачи команды `restore` ещё нет, и утверждение
  осталось бы без теста. Та же форма делегирования, что у доставки дверью
  (BEH-11) выше.
- **And** горячий путь `run` не получает **ни одного** нового синхронного
  ожидания, когда доставлять нечего: прогон `run --task` в каталоге без
  недоставленных правок ведёт себя как до бандла по числу и месту обращений
  к двойнику store — ни одна mutation задачи не ждёт ack между собой и
  следующим шагом. Цену гейта перед closure у подкоманд без платного вызова
...
- **And** восстановленный каталог не дописывает ничего в namespace прогона,
  из которого восстановлен: после restore и первого `run --task <id>` в новом
  каталоге под `run_id` восстановленного прогона у двойника store **не
  появилось ни одного нового ключа checkpoint**, а сам прогон дошёл до своей
  работы (двойник получил его call-start). Наблюдаемое именно такое, а не
  «прогон не отказал»: snapshot снимается раньше ack своего же
  checkpoint-а, поэтому обязательство опубликовать его попадает в snapshot и
  приезжает в новый каталог, где байтов для него нет, — и по общему правилу
  (BEH-48) такое обязательство закрывается пересъёмкой, то есть прогон не
  отказал бы и без всякой очистки, но пересъёмка назвала бы в своём manifest
  чужой `sequence`, и `restore` того же прогона стал бы после этого
  невозможен — петля из уже выполненного восстановления. Новый ключ
  checkpoint под тем `run_id` — красный тест.
