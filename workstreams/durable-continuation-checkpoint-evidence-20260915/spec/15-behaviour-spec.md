...
- **And** первый `run` в восстановленном каталоге спрашивает заново, а не
  ...
  Шаг 5 снимается вторым и только прогоном C: acknowledged checkpoint-а у
  него нет, потому что он убит на границах Given (после ack call-start
  либо после возврата `Popen`), то есть до первой continuation-relevant
  mutation своего прогона. Дверь здесь ни при чём: она закрывает
  **call**, не прогон.
