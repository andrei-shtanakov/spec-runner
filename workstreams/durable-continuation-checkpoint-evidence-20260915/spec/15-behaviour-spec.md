...
- **And** первый `run` в восстановленном каталоге спрашивает заново, а не
  ...
  Шаг 5 снимается вторым и только прогоном C, и снимается не границей
  краша, а **конфигурацией**: C выбрал ровно одну задачу, поэтому
  крашнувший вызов и есть первая continuation-relevant mutation его
  прогона — acknowledged checkpoint-а под его `run_id` нет ни одного. Из
  одних границ Given это не следовало бы: они ограничивают момент
  `os._exit`, а не предшествующую работу, и `run --all`, успевший закрыть
  задачу до краша, уже несёт строку `attempts` и её доставленный
  checkpoint — такой C блокировал бы восстановление шагом 5 совершенно
  правильно. Дверь здесь ни при чём: она закрывает **call**, не прогон.
