...
- **And** выбор задачи этим забором НЕ накрыт, и это предъявлено: `run
  --all` в том же проекте с тем же унаследованным обязательством выбирает
  задачу без ожидания — до своего первого call-start (сайт (а)) ack не
  требуется. Ожидание перед выбором задачи — красный тест: недоставленная
  mutation лежит в локальной DB, и процедура Q-12 видит её без доставки.
