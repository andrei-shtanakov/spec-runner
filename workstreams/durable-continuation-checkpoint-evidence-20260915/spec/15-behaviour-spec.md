...
платного вызова, выбора задачи и успешного завершения), и BEH-48
...
- **And** после `kill -9` в окне между commit-ом mutation и ack (двойник
  ...
  своих** записей и раньше трёх событий, на которых состояние читают, —
  раньше собственного платного вызова, раньше выбора задачи и раньше своего
...
- **And** это предъявлено на трёх прогонах, каждый из которых накрывает своё
  из трёх событий: `run --all` (чужая строка у двойника раньше первого
  `spawn` и раньше того, как прогон взял задачу), **второй** `spec-runner
  budget authorize` (чужая строка раньше его собственного нулевого кода
  выхода) и `spec-runner plan --gated` (чужая строка раньше его `spawn` —
  то есть раньше траты). Свойство, доказанное только на `run`, этот And не
