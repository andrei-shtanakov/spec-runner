...
**объявляет завершение** и не **принимает по store решение, которое не может
вывести локально**. Перед closure ожидание — **гейт**, а не best-effort:
...
Область (в) — решения, авторитет которых лежит в store и которые процесс не
может вывести локально. У проверки (6) `restore` это выполнено буквально:
она судит по индексу workstream-а, локального эквивалента у неё нет.

**Что из этого следует для выбора задачи, по веткам Q-12 — и почему
ожидания там нет ни в одной.** Ветка (1) (маркер `continuation_index:
local`) под область не подпадает вовсе: индексом объявляет себя локальная
DB, недоставленная mutation уже в ней, `budget.effective_limits` и очередь
задач видят её без доставки — store там не единственный источник. Ветка
(2) (маркера нет или он `restored`) под область подпадает: решение
принимается одним `list` индекса workstream-а, и пустой `agent_calls`
там, по её собственному тексту, не доказывает ничего. Но забор в ней
**пуст по построению**: ветка (2) — это свежая, удалённая или приехавшая
из snapshot-а DB, обязательств в ней нет; а унаследованные обязательства
снимает сам `restore` (§ 7.2, шаг 0 и правило их снятия —
spec-runner#528). Ожидание перед выбором задачи — красный тест
(BEH-48), и это утверждение о ветке (1); в ветке (2) ждать нечего, и
отсутствие ожидания там — не исключение из правила, а его пустой случай.
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   ...
   только (б); `restore` — (в) и (б); `run` — (а) и (б), а веткой (2)
   процедуры Q-12 ещё и (в) перед выбором задачи, где очередь пуста по
   построению (§ Q-05, «пустой случай»), поэтому вызов есть, а ожидания
   нет. Прогон из блокирующей
