...
upstream_hashes:
  ...
  behaviour-spec: 1829c952545cafca3c6338076cb1bb476ead11db
---
...
5. Более поздний прогон блокирует восстановление, когда его `subcommand` из
   ...
   **по построению** (§ 2.6), а его `deletions/<ts>.json` — аудит
   ...
   восстановление тут же блокирует. Выход оператора назван в самом отказе:
   восстановить последний `run_id` workstream-а (он печатается) либо, если
   нужен именно ранний, решить это явно. Откат на ранний checkpoint
