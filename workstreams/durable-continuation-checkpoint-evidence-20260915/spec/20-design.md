...
гварда сохраняет тип, который **не** перехватывается `except Exception` в
`execute_task` (`execution.py:1263`), — `RealAgentCallRefused` (ради этого над
ним и стоит `except … raise`, `execution.py:504-512`, `:1255-1261`) либо класс
от `BaseException`, как пояс (`tests/conftest.py:21-30`). Голый
`AssertionError` на `_spawn` стал бы обычным неудачным attempt-ом с `return
False`, тест, ожидающий неуспеха задачи, позеленел бы, и вердикт гварда исчез
бы молча; пояс здесь не страховка — процесс не создавался. Свойство пинует
`tests/test_harness_guards.py` тем же коммитом, что и (2). Двойник
...
| `doctor` | через `execute_task` | `doctor` | сайт не правится, но проба — чужой namespace: `provenance='doctor'`, `task_id=None` (ниже); run-start/closure — § 6 |

`doctor` — единственный сайт, который исполняет чужой путь в чужом
каталоге, и потому единственный с оговоркой. `build_scratch`
(`doctor.py:282-295`) делает временный `project_root` со своим `state_file`,
прогоняет канонную задачу `TASK-001` пробы (`doctor.py:255-263`) через
`execute_task` (`:341`) и удаляет каталог (`:408`); при этом
`copy.deepcopy(base)` оставляет пробе durability-конфиг вызывающего, а ключи
§ 1.3 адресуются `run_id`-ом. Значит call-start и call-result пробы ложатся в
**store проекта** под тем `run_id`, чью индексную строку диспетчер уже
написал (§ 6.2), тогда как attempts, checkpoint и строка `agent_calls`
остаются в удаляемой scratch-DB. Отсюда два требования к этому сайту, оба —
про запись, не про исполнение: call-start несёт provenance `doctor` (значение
словаря требований, `10-requirements.md:115-119`: при «как у сайта» оно не
производится ни одним сайтом вовсе) и `task_id = None` (`TASK-001` пробы —
идентификатор канонной задачи doctor-а, а не задачи проекта). Без второго
ветка (2) процедуры Q-12, листая индекс workstream-а после `reset`, в свежем
клоне или после restore, восстановила бы `open`-строку на **одноимённую
реальную** `TASK-001` проекта и заблокировала бы её до аудируемой двери —
задача, которой платный вызов никогда не касался. Сам open call упавшего
`doctor` при этом не исчезает и предъявляется как любой другой (provenance
`doctor`, `task_id` пуст): платный вызов действительно был начат и не закрыт,
снимается он той же дверью § 2.5.
...
5. Более поздний прогон, **оставивший в namespace изменения, которых
   snapshot не несёт**, — тоже отказ `needs-human`, но с другим сообщением:
   восстановленное состояние старше этих изменений (`budget authorize`,
   ...
   Критерий — **свойство прогона, а не его имя**: блокирует тот, чьи
   изменения snapshot не несёт. Прогон, не менявший continuation-state, на
   этом шаге не в счёт, каким бы платным он ни был: `evidence close-call` и
   `evidence purge` (§ 2.5, § 6.2 — `close-call` лишь возвращает задачу в
   выбираемые, `purge` лишь удаляет объекты store), `plan`, `review-pr`,
   `doctor` (§ 2.6 — платят, но задач ledger-а не выбирают) и **сам
   `restore`**: он входит в `PAYING_SUBCOMMANDS` (§ 6.2), поэтому диспетчер
   кладёт его run-start и индексную строку **до** вызова handler-а и до
   этой проверки, и правило, ключующееся на «любой более поздний прогон»,
   отказывало бы первому же `restore` от его собственной записи. Перечень
   здесь — иллюстрация вывода, а не сам критерий: подкоманда, попавшая в
   `PAYING_SUBCOMMANDS` позже, ложится в ту или другую сторону по тому же
   свойству, без правки этого списка. Блокируют, стало быть,
   `run`/`retry`/`watch`, `budget authorize` и
   `tdd abandon|repair|resume|release` — и только они (BEH-09, последние
   And). Без этого вывода у FR-05 не остаётся ни одного достижимого пути:
