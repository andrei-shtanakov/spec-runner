...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   половине закрытого перечня **и** под его `run_id` опубликован хотя бы
   один checkpoint (`runs/<run_id>/checkpoints/…`, § 1.3 — то есть прогон
   хоть раз позвал `after_mutation`, § 3.1). Ключ составной, потому что ни
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18).**
   Публикация асинхронна: процесс ждёт publisher только перед call-start и
   перед closure (Q-05). Прогон из блокирующей половины, убитый в окне между
   записью mutation в DB и доставкой checkpoint-а в store — например
   `budget authorize` под `kill -9`, — в store checkpoint-ов не имеет, и
   потому этим шагом **не** блокирует: restore более раннего прогона
   применяется, а его правка `budget_authorizations` теряется молча, без
   упоминания в отказе. Строго говоря «ключей нет» доказывает не «работы не
   ...
   половина, не работал бы ни разу. Из двух зол выбрано узкое: потеря в окне
   доставки у подкоманд без платного вызова (`budget authorize`,
   `tdd release`) против недостижимости FR-05 во всех конфигурациях с
   упавшим прогоном. Закрывается это не здесь, а сужением самого окна
   (синхронный ack mutation-checkpoint-а для таких подкоманд) — отдельной
   задачей.

   **Блокирующая половина:** `run`, `retry`, `watch`, `plan`, `review-pr`,
   `budget authorize`, `tdd abandon|repair|resume|release`. Прогон из неё с
   хотя бы одним опубликованным checkpoint-ом даёт `needs-human`, и
   сообщение другое, чем на шаге 4: восстановленное состояние старше его
   изменений, продолжение с него потеряло бы их молча. Без опубликованных
   ...
   **по построению** (§ 2.6), а его `deletions/<ts>.json` — аудит
   ...
   вводит, и это его известный пробел. Кандидат при этом существует всегда, когда шаг 5
   сработал: блокирующий прогон по второму условию ключа сам несёт
   acknowledged checkpoint, а самый поздний из блокирующих по построению не
   имеет блокирующих после себя — он и печатается. Вырожденный случай «ни
   одного подходящего прогона» отказ всё же различает: тогда он говорит
   «более позднего восстановимого прогона нет» и выхода не предлагает.
   Отдельно отказ называет, чего checkpoint выхода может не нести: материал,
   записанный прогоном **после** его последнего checkpoint-а, туда не
   попадёт — так у `plan`, дописывающего `tasks.md` уже после платного
   вызова (запись файла — не `record_*` и `after_mutation` не зовёт). Это
   не мешает выходу быть исполнимым, но оператор должен решать
   информированно, а не обнаруживать пропажу после восстановления. Откат на ранний checkpoint
