...
может вывести локально; ею НЕ покрыт выбор задачи процедурой Q-12, и это
не исключение, а та же граница: недоставленная mutation уже лежит в
локальной DB, `budget.effective_limits` и очередь задач видят её без
доставки, поэтому store там не единственный источник. У `restore` иначе:
проверка (6) судит по индексу workstream-а, локального эквивалента у неё
нет.
