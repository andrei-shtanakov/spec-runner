...
5. Более поздний прогон блокирует восстановление, когда его `subcommand` из
   ...
   **по построению** (§ 2.6), а его `deletions/<ts>.json` — аудит
   ...
   обязан быть **исполнимым**: печатается последний прогон workstream-а, у
   которого есть acknowledged checkpoint, — не просто «последний по
   `started_at`». Последним по времени в индексе на момент шага 5 лежит
   run-start самого текущего `restore` (§ 6.2 кладёт его до handler-а), а
   перед ним может стоять холостой прогон; ни тот, ни другой восстановить
   нечем — digests берутся из последнего acknowledged checkpoint-а, а у них
   его нет, и legacy-проверка отбила бы такой `run_id` fail-closed
   (BEH-21 (а)). Поэтому собственная запись текущего invocation из выбора
   исключена, и прогоны без acknowledged checkpoint-а — тоже. Если
   восстановимого прогона в workstream-е нет вовсе, отказ так и говорит
   («более позднего восстановимого прогона нет») и выхода не предлагает:
   решение — за оператором, явно. Откат на ранний checkpoint
