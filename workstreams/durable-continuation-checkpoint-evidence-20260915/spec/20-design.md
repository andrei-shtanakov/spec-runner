...
Перед closure это **гейт**, а не best-effort: недоставленный checkpoint даёт
exit 2, и подкоманда без платного вызова успешной не завершается.
...
предыдущего acknowledged. Между точками drain процесс делает то,
что делал (тесты, lint, git, сборка prompt-а) — это и есть выигрыш от
асинхронности.

**Третьей точки нет, и это решение, а не пропуск (spec-runner#527).**
Требование #527 — «один синхронный drain перед успешным завершением
команды, таймаут → exit 2, а не ложный успех» — выполняет точка (б): она
стоит в `finally` диспетчера, то есть сразу после handler-а, и по правилу
§ 6.3 её таймаут **перекрывает** выведенный kind — closure `failed`, exit 2.
Окно между последней mutation подкоманды и (б) — это хвост handler-а, у
`budget authorize` и `tdd …` печать результата и возврат, а не работа.
Отдельная точка «сразу после mutation» его не сужает заметно и стоит дорого:
...
Поэтому ожидание одно на invocation, и оно там, где уже было.
...
доставлено», чтобы человек не читал stdout как результат; (3) pending-outbox
(ниже) превращает `kill -9` в окне в позднюю доставку — это и есть настоящее
лекарство от потери, а не ещё одно ожидание.
...
недоставленного checkpoint-а, а следующий прогон в этом каталоге отправляет
его **раньше собственных** записей — и потому раньше двух событий, после
которых прогон читают вне машины: раньше своего платного вызова и раньше
своего успешного завершения (§ 3.5; «прежде любой другой работы» резолюция
не требует, и четвёртой точки drain доставка не заводит).
Доставка идёт тем же publisher-ом, в порядке
...
общий для всех mutation: у `run` он закрывает то же окно
между mutation и следующим drain.

**Строка, для которой локальных байтов нет.** Окно между commit-ом
транзакции и созданием каталога snapshot-а существует (snapshot снимается
после commit-а), и процесс, убитый в нём, оставляет строку без единого
файла: доставлять нечего, потому что checkpoint не был снят вовсе. Отказ
здесь был бы худшим из возможных — строку удаляет только ack, так что
каталог перестал бы запускаться навсегда. Правило поэтому другое: такая
строка закрывается **пересъёмкой** — publisher снимает checkpoint текущего
состояния DB (оно содержит ту mutation и всё, что легло позже), его manifest
несёт пропущенные `sequence` в `supersedes`, и строки закрываются им. Это
законно по FR-03: для continuation обязателен только **последний**
checkpoint (RK-05), а промежуточная точка, которой не существовало, и не
обещана. Отказ остаётся только на случай, когда и пересъёмка невозможна
(DB недоступна) — тогда работает § 5, spool и degraded mode. Чего он не закрывает: доставку выполняет
...
решается, — сайты записи о пробе не знают. (2) Ждать ack
`after_mutation` **не** ждёт ни у одной подкоманды: синхронного drain внутри
неё нет (Q-05, «третьей точки нет»). Ожидание перед успешным завершением
даёт точка (б) в `finally` диспетчера — одно на invocation, там же, где
выводится kind closure, и потому одинаково для подкоманды, добавленной
после бандла. Ожидание внутри `after_mutation` подняло бы `Refusal` из
середины многошагового handler-а — между записями, порядок которых
`remedy.py` выбрал так, чтобы обрыв не оставил запрещённого состояния. (3) Строка `checkpoint_outbox` — её пишет не `after_mutation`, а сама
...
собственных**, а ждут их те же две точки drain, что и всё остальное (Q-05).
Отсюда порядок, который получается сам и который предъявляют сценарии:

- **до любого платного вызова** — точка (а) стоит перед call-start, и
  очередь к этому моменту наполнена: state DB проекта открывает
  `RunContext.start()` в диспетчере, до handler-а (§ 6.2, § 7.4), у любой
  платящей подкоманды, так что инвариант «между потраченным долларом и
  последним acknowledged checkpoint-ом нет неподтверждённой mutation»
  распространяется и на чужую недоставленную. Вторая линия на случай, когда
  DB открылась позже (degraded-старт, недоступная на момент `start()` DB):
  `Publisher.publish(CallStart)` шага 4 идёт **той же** FIFO-очередью и
  встаёт позади чужих строк, а его ack синхронен, поэтому `_spawn` шага 5 не
  наступает, пока они не доставлены. Отсюда требование к реализации, а не
  пожелание: публикация call-start обязана идти через тот же `Publisher`;
  второй синхронный путь в store мимо очереди обгонял бы чужие строки и
  тратил деньги раньше доставки;
- **до успешного завершения** — точка (б) перед closure, у всех: команда не
  завершается успешно, пока чужая строка не acknowledged, потому что она
  стоит в очереди раньше собственных (Q-05: таймаут (б) перекрывает kind —
  closure `failed`, exit 2).

Точек drain две (Q-05), третьей доставка не заводит — и
**«раньше выбора задачи» в этом перечне намеренно нет**. Рубеж
`cli._run_start_gate` остаётся тем, чем был (replay spool → процедура Q-12,
§ 2.4): процедура решает по open calls в store, а не по checkpoint-ам, и
недоставленный чужой checkpoint на выбор задачи в **этом** каталоге не
влияет — mutation, которую он несёт, уже в локальной DB, и читатели вроде
`budget.effective_limits` видят её и без доставки. Доставка нужна тому, кто
читает прогон **вне** машины (шаг 5 § 7.2, restore), поэтому её и хватает
накрыть двумя событиями: тратой и собственным успешным выходом.

Это слабее, чем «прежде любой другой работы процесса», и ровно настолько,
насколько нужно. Ни одной
...
**Кто не доставляет — ровно два класса, и перечень выводится, а не
перечисляется.** Первый: **read-only команды** — `status`, `costs`,
`validate`, `report`, `evidence <run_id>` state DB открывают, но
`Publisher`-а не создают и ни одной точки drain не проходят, а BEH-04 и
BEH-13 требуют от них «ничего в store». Второй: **прогон, умерший до своей
первой точки drain**. Больше никого: `RunContext.start()` открывает state DB
проекта, когда она доступна (пишет meta `last_run_id`/`last_pipeline_id`,
§ 7.4), и делает это до handler-а — значит у **каждой** платящей подкоманды
очередь наполнена уже к шагу 1 seam-а, а closure (§ 6.3) проходит точку (б)
даже у прогона, который ничего больше не сделал. Поэтому `doctor` чужую
строку как раз доставит (у его пробы своя DB, но диспетчер открыл DB
проекта), и `plan`, отказавший до платного вызова, — тоже, на своей closure.
Единственная платящая подкоманда вне этого — `restore`: он исходную DB не
открывает вовсе (§ 7.2 читает store и `--into`-клон), и это названный вход
остатка (§ 7.2, FR-05), а не упущение. Ожидание — ремонт, а не цена каждой
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   ...
   владельца 2026-09-18, вторая половина): гейт на точке (б) делает успешное
   завершение невозможным без доставки, а `checkpoint_outbox`
   ...
   завершения — потому что каталога больше нет (машина потеряна вместе с
   outbox-ом), потому что там ничего не запускали, или потому что запускали
   только то, что state DB через `Publisher` не открывает: read-only
   команды и сам `restore`. Он
...
создаются (BEH-17). Сразу после укладки DB `apply` правит в ней **два**
места, и оба — не `record_*`, так что `after_mutation` не зовут. Правит он
их **своим** соединением, до того как DB откроется общим путём (тем, что
ставит незакрытые строки в очередь publisher-а, § 3.5): порядок здесь — не
деталь реализации, а условие, без которого первая же точка drain самого
`restore` упёрлась бы в доставку унаследованных строк. Первое:
таблица `checkpoint_outbox` **очищается**. Строка обязательства пишется
транзакцией mutation, а удаляется по ack, тогда как snapshot снимается
между тем и этим, — значит DB-snapshot любого checkpoint-а несёт незакрытую
строку **своего же** checkpoint-а. Восстанавливаемся мы из acknowledged
checkpoint-а (§ 7.2 начинается с его digests), то есть всё, на что
указывают унаследованные строки, доставлено по определению; а локальных
копий checkpoint-ов в новом каталоге нет по построению (они лежат под
`.executor-*` исходного и в состав checkpoint-а не входят, § 3.2, § 3.3).
Не очистить их означало бы, что первый же `run` в восстановленном каталоге
ставит в очередь строку, байтов для которой нет, и отказывает
`instrument`/exit 2 — то есть восстановленный каталог не запускается ровно
в том сценарии, ради которого restore и существует. Второе: один ключ
...
- MUST NOT assert: результат `grep` по исходнику на `subprocess.run`
  ...
  служебной таблицы; wall-clock цены гейта перед closure внутри CI-теста — её
