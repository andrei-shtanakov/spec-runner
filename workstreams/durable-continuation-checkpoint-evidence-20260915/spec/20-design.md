...
неё нет (Q-05, «точки сразу после mutation нет»: ни один сайт правила не
наступает МЕЖДУ записями многошагового handler-а — сайт (а) стоит внутри
handler-а, но перед платным вызовом, а не между mutation). Ожидание перед успешным
...
вычисление следующего безопасного шага ≤ 60 s») названо прямо, потому что
ожидание здесь может занять до тех же 60 s: бюджет NFR-03 меряется на
наборе **без недоставленных обязательств** — то есть на том, где сайт (в)
возвращается сразу. Медленная, но успешная доставка (ack на 40-й секунде)
этот бюджет превысит, и это принято: альтернатива — читать store раньше
доставки, ради которой забор и поставлен. Отдельного бюджета ожидания
бандл не вводит; при появлении механизма (#528) его стоит назвать в AC-42,
который меряет NFR-03.
