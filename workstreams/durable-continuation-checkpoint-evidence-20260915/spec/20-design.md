...
состояния DB (оно содержит ту mutation и всё, что легло позже) и закрывает
ею строку. Это законно по FR-03: для continuation обязателен только
**последний** checkpoint (RK-05), а промежуточная точка, которой не
существовало, и не обещана.

Пересъёмка публикуется **под `run_id` того прогона, который её снял**, а не
под `run_id` строки: checkpoint принадлежит прогону, чьё состояние он
содержит, а `sequence` мёртвого прогона уже израсходован — публикация под
ним дала бы либо дубль номера (AC-11), либо запись в чужой namespace с
чужими правками внутри. Чтобы правка всё-таки стала видимой шагу 5, manifest
несёт поле **`absorbed`** — перечень `{run_id, sequence}` закрытых
обязательств; шаг 5 проверки (6) (§ 7.2) читает acknowledged checkpoint с
`absorbed` как признак работы **и** для каждого absorbed-прогона, так что
`budget authorize`, убитый до snapshot-а, блокирует восстановление так же,
как если бы доставил сам. Остаток назван: если пересъёмку выполнил прогон из
**неблокирующей** половины (`evidence purge`, `evidence close-call`), признак
появляется у absorbed-прогона и работает; если её не выполнил никто —
работает принятый пробел. Отказ остаётся только на случай, когда и
пересъёмка невозможна (DB недоступна) — тогда работает § 5, spool и
degraded mode. Чего он не закрывает: доставку выполняет
...
Ни обязательств публикации проба не оставляет: `record_*` при заполненном
`probe_provenance` строки `checkpoint_outbox` не пишет (§ 3.1) — иначе
правило пересъёмки Q-05 опубликовало бы снимок scratch-каталога под `run_id`
вызывающего.
...
её DB удаляется вместе с каталогом. Поле читают три места, и третье —
`record_*`: при заполненном `probe_provenance` строка `checkpoint_outbox` не
пишется вовсе (ниже, (3)). Иначе scratch-DB несла бы обязательства, для
которых байтов нет по построению (`after_mutation` вышла), и правило
пересъёмки (Q-05) опубликовало бы **снимок scratch-каталога** под `run_id`
внешнего invocation-а — ровно то, что BEH-47 объявляет красным тестом, — а
`sequence` пробы, считаемый в её собственной DB, продублировал бы номера
проекта под тем же `run_id`. Читатели поля, таким образом: `record_*`,
`after_mutation` и `export_attempt` (§ 6.4), и это единственные три места,
где область пробы вообще что-то решает. (2) Ждать ack
...
зафиксированное обязательство и лишь исполняет его; ack удаляет строку. При
заполненном `config.probe_provenance` строка не пишется — у пробы нет ни
checkpoint-ов, ни обязательств их опубликовать (см. (1)); постановка строк в
очередь (§ 3.5) DB пробы поэтому не касается, и это свойство её данных, а не
свойство порядка открытия соединений.
...
пока нет), `absorbed[]` (`{run_id, sequence}` обязательств, закрытых этой
пересъёмкой; пусто у обычного checkpoint-а — Q-05), `digests{file: sha256}`, `excluded[]` (lock, `.<prefix>spec.lock`,
...
**Доставляющий прогон — тот, кто создал `Publisher` и прошёл хотя бы одну
точку drain в этом каталоге.** Признак именно такой, потому что доставка —
это очередь плюс ожидание: без `Publisher`-а строки некуда поставить, без
точки drain — некому дождаться. Отсюда два класса недоставляющих, и оба
выводятся, а не перечисляются. Первый: **read-only команды** (`status`,
`costs`, `validate`, `report`, `evidence <run_id>`) — state DB открывают, но
`Publisher`-а не создают, и BEH-04/BEH-13 требуют от них «ничего в store»;
успешно завершившийся `status` доставки поэтому не означает. Второй:
**прогон, умерший до своей первой точки drain**. Больше никого:
`RunContext.start()` открывает state DB
...
строку как раз доставит (у его пробы своя DB и своих обязательств нет — § 2.7,
но диспетчер открыл DB проекта), и `plan`, отказавший до платного вызова, —
тоже, на своей closure. `restore` при этом делится по каталогу, в котором
его запустили, и это не оговорка, а следствие признака: запущенный **в
пострадавшем каталоге** он доставляет (диспетчер открыл его DB, точка (б)
перед closure чужие строки дождётся); запущенный на другой машине — нет,
потому что того каталога там просто нет, и это названный вход остатка
(§ 7.2, FR-05).

**Гейт перед closure — про свои checkpoint-ы, не про чужие строки.**
Недоставка **своего** checkpoint-а делает прогон неуспешным (exit 2 — это и
есть #527). Недоставка **чужой** унаследованной строки — нет: иначе авария
одного прогона роняла бы любую следующую команду в каталоге, включая тот
самый `restore`, которым оператор из этой аварии выходит, и «дверь →
restore» снова стала бы недостижимой. Чужие строки едут на тех же точках
drain best-effort: успели — шаг 5 их видит, не успели — они остаются на
следующий раз, а прогон завершается своим исходом. Ожидание — ремонт, а не цена каждой
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **acknowledged** checkpoint — свой либо чужой, назвавший его в `absorbed`
   (Q-05: пересъёмка закрывает обязательство прогона, не успевшего снять свой
   snapshot, и для этого шага она — его checkpoint) (§ 1.3, § 3.5: manifest
   кладётся последним,
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   ...
   прогона не отработал ни один **доставляющий** прогон (§ 3.5: создал
   `Publisher` и прошёл хотя бы одну точку drain) — потому что каталога
   больше нет (машина потеряна вместе с outbox-ом), потому что там ничего не
   запускали, или потому что запускали только read-only команды, канала
   публикации не открывающие. Он
...
Не очистить их — не отказ (строка без байтов закрывается пересъёмкой, Q-05),
а **петля**: первый же `run` в восстановленном каталоге снял бы пересъёмку,
назвав в её `absorbed` `sequence` прогона A, — и шаг 5 прочитал бы её как
работу A **после** его собственного checkpoint-а, то есть `restore <A>`
стал бы невозможен из-за самого факта уже выполненного `restore <A>`.
Наблюдаемое поэтому не «прогон не отказал» (это истинно и без очистки), а
«под `run_id` прогона A после restore и первого `run` не появилось ни одного
нового ключа checkpoint» (BEH-17, AC-15). Второе: один ключ
