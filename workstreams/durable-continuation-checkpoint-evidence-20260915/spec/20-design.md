...
- **fail-closed — забор не нужен.** Ветка (1) Q-12 (маркер
  ...
  **закрытие** строки ledger-а, поэтому устаревший store отвечает «всё
  ...
  цена без гарантии. Ожидание перед выбором задачи здесь — красный тест
  (BEH-48).
...
упорядочена по `sequence`; `drain(timeout)` зовётся на трёх сайтах правила
Q-05 (не тратит / не объявляет завершение / не решает по store); manifest
