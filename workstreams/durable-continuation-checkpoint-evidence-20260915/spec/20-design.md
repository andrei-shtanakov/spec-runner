...
upstream_hashes:
  ...
  behaviour-spec: dda69108654dda226dc74e47efaf86b7321cb28e
---
...
5. Более поздний прогон — блокирует он восстановление или нет, решает
   **закрытый перечень подкоманд**. Ключ выбран так осознанно, а не по
   лени: единственное, что проверка (6) может прочитать о чужом прогоне, —
   его `run-start.json` (§ 6.2), а из относящегося к делу там только
   `subcommand`. Поля «этот прогон добавил continuation-state» не несут ни
   run-start, ни индекс workstream-а (§ 1.3), а косвенные признаки
   инвертированы в обе стороны: у неблокирующей двери `evidence close-call`
   checkpoint **есть** (её шаг close идёт через `after_mutation`, § 2.5), а у
   блокирующего `plan` его нет вовсе; `attempts/` не отличает блокирующий
   `budget authorize` от неблокирующей двери — attempt-ов нет ни у того, ни у
   другой (§ 6.3). Критерий, сформулированный как свойство прогона, был бы
   поэтому невычислим, и реализатору осталось бы угадывать.

   **Блокируют:** `run`, `retry`, `watch`, `plan`, `review-pr`,
   `budget authorize`, `tdd abandon|repair|resume|release`. Отказ —
   `needs-human`, и сообщение другое, чем на шаге 4: восстановленное
   состояние старше их изменений, продолжение с него потеряло бы их молча.
   **Не блокируют:** `evidence close-call`, `evidence purge`, `doctor`,
   `restore`.
   ...
   потому, что дописывает задачи в `tasks.md` (`cli_plan.py:878`) вне
   всякого checkpoint-а: восстановленный каталог этого материала не несёт.
