...
Свойство даёт три сайта, где процесс ждёт publisher-а с таймаутом
...
объявить его обязана та же задача, что вводит сами обязательства. Место выбрано по тому, что забор защищает: устаревший store
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   ...
   правила Q-05 — перед call-start, перед closure и перед чтением store у
   `restore` (сайты (а), (б), (в); (б) — гейт: недоставленный checkpoint
   даёт exit 2). Сколько сайтов проходит invocation — зависит от того,
   какие из трёх действий он совершает: `budget authorize` и `tdd …` —
   только (б); `restore` — (в) и (б); `run` — (а) и (б). Вызова drain
