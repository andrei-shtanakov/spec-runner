...
неё нет (Q-05, «точки сразу после mutation нет»). Сайт (а) у invocation-а
с несколькими платными вызовами наступает и после записей вокруг
предыдущего вызова — это не то же самое, что отвергнутая точка: его отказ
приходит **до spawn**, до траты, и за ним остаются только завершённые
записи, тогда как ожидание внутри `after_mutation` рвало бы одну
логическую операцию между её записями. Ожидание перед успешным
завершением даёт сайт (б) в `finally` диспетчера — одно на invocation,
там же, где
...
упорядочена по `sequence`; `drain(timeout)` зовётся на сайтах правила
Q-05 (не тратит / не объявляет завершение / не решает по store) — в этой
волне на двух, (а) и (б); третий, (в), объявлен § 7.2 и приезжает с
механизмом обязательств (spec-runner#528); manifest
...
Сайт (б) — **гейт**: недоставленный checkpoint даёт closure `failed` и
exit 2, поэтому подкоманда без платного вызова успешной не завершается
(§ 6.3, FR-03). Сайт (в) — забор на чтение перед проверками `restore`
(§ 7.2): дренаж после чтения бесполезен, вердикт уже посчитан. Ожиданий у
invocation-а столько, сколько сайтов правила он проходит, — число mutation
подкоманды его не увеличивает. Решение ждать принимает не один хозяин, а
сам сайт: (а) — seam платного вызова (`paid_call.execute`, шаг 1), (б) —
диспетчер в `finally`, (в) — `restore.plan`. Общее у всех трёх не
вызывающий, а то, чего среди них нет: `after_mutation` не ждёт ни на одной
записи (Q-05, «точки сразу после mutation нет»).
...
достигает; строит их одна задача. Обязательства здесь **унаследованные**: прогон, убитый между commit-ом
своей mutation и доставкой её checkpoint-а, оставил их у себя. **Каким
носителем они попадают в очередь `restore` — вопрос #528, а не факт этого
бандла**, и вот почему он не тривиален: рабочая форма, записанная в #528
(«незакрытые обязательства встают в очередь publisher-а при первом за
процесс открытии state DB»), на `restore` не переносится — он локальную DB
не читает вовсе, только store (§ Механика, резолюция Q-12: snapshot мог
быть снят раньше строки). Любой носитель здесь будет исключением из этого
правила — например целевое чтение одной outbox-таблицы, не состояния, — и
объявить его обязана та же задача, что вводит сами обязательства.

Место выбрано по тому, что забор защищает: устаревший store искажает не
только вердикт проверки (6), но и **сам выбор материала** — проверка (2)
берёт digests *последнего acknowledged* checkpoint-а, и без доставки
последним считался бы не тот.
...
обязательства есть — то есть когда носитель (выше) донёс их из дерева
убитого прогона. Каталог удалён, восстановление идёт на другой машине или
из свежего клона — локальных байтов обязательства нет по построению,
доносить нечего, и забор сторожит пустую очередь. Это не
...
**принятый** пробел FR-05. Здесь фиксируется только место, раньше
которого читать store нельзя, — и это ровно то, чего механизму не хватало,
чтобы на пути «crash → restore» вообще иметь смысл. Дренаж после любого из этих чтений
...
молчаливой потери. **Отношение к NFR-03** («после загрузки валидация и
вычисление следующего безопасного шага ≤ 60 s») названо, но НЕ решено
здесь: ожидание на этом сайте может занять до тех же 60 s, и медленная,
но успешная доставка (ack на 40-й секунде) порог превысит. Объявлять это
принятым дизайн не вправе — продуктовые решения upstream-а он не
пересматривает (вступление § Дизайн), а NFR-03 сформулирован без оговорок
и меряется критерием AC-42. Поэтому: бюджет ожидания на пути `restore`
назначает задача, которая вводит механизм (spec-runner#528), и она же
вносит оговорку туда, где ей место, — в NFR-03 и/или AC-42. До тех пор
сайт (в) возвращается сразу (обязательств нет по построению), и NFR-03
этим ожиданием не затрагивается вовсе.
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   чем он сужен.** Публикация асинхронна: процесс ждёт publisher на сайтах
   правила Q-05 — в этой волне перед call-start и перед closure (сайты
   (а) и (б); (б) — гейт: недоставленный checkpoint даёт exit 2); сайт
   (в) перед первым чтением store у `restore` правилом объявлен, но
   приезжает с механизмом обязательств (spec-runner#528). Сколько сайтов
   проходит invocation сегодня, по одиннадцати позициям `PAYING_SUBCOMMANDS`
   (§ 6.2): (б) — у всех; (а) дополнительно у всех, кто платит, то есть у
   `run`, `retry`, `watch`, `plan`, `review-pr` и `doctor` (шаг 1 единого
   seam-а `paid_call.execute`, § 2.4), и по разу перед каждым их платным
   вызовом. Только (б) — у `budget authorize`, `tdd …`, `evidence
   close-call`, `evidence purge` и `restore`. Вызова drain
   перед выбором задачи нет ни в одной ветке Q-12, и § 2.4 его не ставит:
   в ветке (1) правило его не требует (ошибка устаревания там
   fail-closed), в ветке (2) оно выполнено вхолостую — обязательств у
   процесса нет по построению. Это не вывод ветки (2) из-под правила:
   появись путь, приводящий в неё с непустой очередью, забор потребовался
   бы там по самому правилу, а не по новому решению. Прогон из блокирующей
   ...
   усиление» заявки #527), бандл по-прежнему не вводит — сам механизм
   ...
   mutation, их подъём в очередь при первом открытии DB, правило для
   обязательства без локальных байтов и снятие унаследованных обязательств
   при `restore`. Fail-closed по-прежнему не читается — по той же причине,
   что выше.
...
| `src/spec_runner/restore_cmd.py`, `evidence_cmd.py` (новые) | `restore` (`plan`/`apply`, порядок проверок, next step, `--experimental`, `--json`), проверка (6) по индексу workstream-а (§ 7.2), запись meta `continuation_index: restored` при `apply` (§ 7.3), `evidence` (`collect`, `close-call`, `purge`), `retention.py`; **drain перед первой проверкой** (сайт (в) правила Q-05) объявлен в § 7.2, но в объём этой задачи не входит — приезжает с механизмом обязательств (spec-runner#528). Closure этих трёх подкоманд пишет диспетчер по коду их выхода — своих сайтов closure у них нет (§ 6.3) | BEH-09, 11, 19…21, 30, 36…38, 40, 42, 46 |
...
| `src/spec_runner/remedy.py`, `budget_cmd.py` | не правятся этим дизайном: их closure пишет диспетчер по коду выхода (§ 6.3), и тот же диспетчер ждёт ack их mutation-checkpoint-а на точке (б) — гейт перед closure, а не сайт ожидания внутри handler-а (§ 3.1, Q-05: точки «сразу после mutation» нет, иначе `Refusal` рвал бы многошаговый `remedy.py` посередине) | BEH-46, 48 |
