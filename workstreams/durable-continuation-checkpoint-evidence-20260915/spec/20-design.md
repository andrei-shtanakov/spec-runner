...
ДО того, как handler начал работу, поэтому ни один из этих аргументов к
нему не применим. Переносить отказ с одной точки на другую по совпадению
...
подкоманды его не увеличивает: решение ждать принимает диспетчер на
границах invocation-а, а не `after_mutation` на каждой записи (Q-05,
«точки сразу после mutation нет»).
...
молчаливой потери. Бюджет NFR-03 («валидация и вычисление следующего шага
≤ 60 s») этим ожиданием не нарушается: таймаут ack — тот же
`durability.checkpoint_ack_timeout_seconds` (дефолт 60 s = NFR-02), и его
исчерпание даёт отказ, а не затянувшуюся валидацию; NFR-03 меряет путь,
дошедший до вердикта.
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **Известный пробел, принятый осознанно (решение владельца 2026-09-18), и
   ...
   проходит invocation сегодня: `budget authorize` и `tdd …` — только
   (б); `restore` — только (б); `run` — (а) и (б). Вызова drain
