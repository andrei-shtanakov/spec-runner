...
upstream_hashes:
  ...
  behaviour-spec: ad524683005acd366942175cf5fa8800727855fb
---
...
5. Более поздний прогон блокирует восстановление, когда сошлись **два**
   ...
   **только перечень** блокировал бы холостой прогон — `run --all`, которому
   ...
   последнего acknowledged checkpoint-а, которого у холостого прогона нет.
   ...
   **Блокирующая половина:** `run`, `retry`, `watch`, `plan`, `review-pr`,
   ...
   хотя бы одним checkpoint-ом даёт `needs-human`, и сообщение другое, чем
   на шаге 4: восстановленное состояние старше его изменений, продолжение с
   него потеряло бы их молча. Без checkpoint-а прогон из этой половины
   восстановлению не мешает.
