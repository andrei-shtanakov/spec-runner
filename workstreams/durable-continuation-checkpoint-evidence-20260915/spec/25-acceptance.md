...
стоит в журнале **раньше** первого `list` индекса workstream-а, а с
отклоняющим двойником `restore` завершается exit 2, `--into` остаётся
пустым и snapshot не применён. Обратный порядок (`list` раньше ack) или
...
невыполненный критерий. Ветка (2) под область (в) подпадает, но забор в
