...
namespace не добавивший, отказа не даёт, и это предъявлено на двух, каждый
...
где в `pr_agent_calls`) и сам `restore`, выполненный второй раз в новый
пустой `--into` (повторный запуск в занятый каталог отказывает раньше, на
проверке каталога). Отказ на любом из двух — невыполненный критерий: он
