...
работы: правка ранее убитого `budget authorize` (AC-46), оставшаяся
недоставленной, уходит в store на вызове `close-call` — раньше
checkpoint-а самой двери и раньше её кода выхода, — и последующий
`restore` более раннего прогона отказывает `needs-human` шагом 5; дверь,
завершившаяся с той правкой всё ещё недоставленной, невыполненный критерий, потому что тогда канонная
...
#### AC-46: Подкоманда без платного вызова не сообщает об успехе раньше ack своего checkpoint-а · verification: test
traces: [FR-03, FR-05, FR-07]
scenarios: [BEH-48]

Наблюдаемый знак: в `spec-runner budget authorize … --reason …` и
`spec-runner tdd release … --reason …` с исправным двойником store ack
mutation-checkpoint-а стоит в журнале раньше строки успеха и раньше выхода с
кодом 0; с двойником, отклоняющим ack, обе завершаются exit 2, stderr
называет недоставленный `sequence` и `checkpoint_id`, и нулевой код у любой
из двух — невыполненный критерий. Mutation при этом в DB есть (она закоммичена раньше);
носитель обязательства опубликовать — решение design-стадии, и критерий его
не называет. После `kill -9` в окне между commit-ом
mutation и ack недоставленный checkpoint доезжает: следующий прогон в том же
каталоге отправляет его раньше своих записей и раньше двух событий, после
которых прогон читают вне машины, — раньше своего платного вызова и раньше
своего успешного завершения, — с тем же `checkpoint_id` и
идемпотентно. Доставка раньше **выбора** задачи критерием не является (это
была бы четвёртая точка ожидания, которой дизайн не вводит). Предъявлено на
трёх прогонах, накрывающих оба события
...
объёме). Горячий путь `run` не получает ни одного нового синхронного ожидания, когда
доставлять нечего: `run --task` в каталоге без недоставленных правок ведёт
себя как до бандла по числу и месту обращений к двойнику store, и ожидание
на каждую mutation задачи — невыполненный критерий (RK-01). Цену третьей
точки drain меряет AC-41 (бенчмарк), не этот критерий.
...
8. **Резолюции design не противоречат критериям.** Q-02 (ack = возврат
   ...
   (локальный snapshot синхронно, publisher с drain перед call-start, перед
   closure и — у подкоманд без платного вызова — сразу после mutation,
   manifest кладётся последним) — AC-11 считает checkpoint
   полученным по ack двойника, AC-29 фиксирует лишь acknowledged id в
   closure, AC-46 — что третья точка наступает раньше сообщения об успехе.
...
подкоманда без платного вызова вправе сообщить об успехе до ack своего
