...
Полнота перечня проверена подсадкой, а не чтением: подкоманда, добавленная
в `PAYING_SUBCOMMANDS` и не отнесённая ни к одной половине, красит тест
`set(PAYING_SUBCOMMANDS) == BLOCKING | NON_BLOCKING`; зелёный тест на такой
подсадке — невыполненный критерий. Применённый
