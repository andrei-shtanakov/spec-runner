...
`review-pr` (его `pr_*` continuation-relevant по § 3 требований), `plan`
(дописывает задачи в `tasks.md`) и `doctor` (проба гоняет `execute_task`,
пишущий `attempts`) — тоже отказывает `needs-human` с именем последнего
`run_id`; более поздний прогон, ничего такого не оставивший, отказа не даёт,
и это предъявлено на двух, каждый из которых оставляет свою строку в индексе
workstream-а: аудируемая дверь `evidence close-call` (её шаг close пишет
`agent_calls` и публикует checkpoint — ни то, ни другое в перечень § 3
требований не входит) и сам `restore`, выполненный дважды подряд (его
собственный run-start ложится в индекс раньше этой проверки). Отказ на любом
из двух — невыполненный критерий: он означает, что проверка ключуется на
перечне имён подкоманд или на факте checkpoint-а, а не на свойстве «оставил
изменения, которых snapshot не несёт». Применённый
