...
перед платными вызовами плюс одна перед closure; ожидание на каждую mutation
на горячем пути `run` — невыполненный критерий (RK-01). Непустой на старте
outbox даёт ровно одно ожидание — доставку незакрытой строки до выбора
задачи, — и только один раз на строку: второй `run` подряд не ждёт.
