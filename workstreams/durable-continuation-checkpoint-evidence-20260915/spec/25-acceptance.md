...
критерий. Mutation в DB есть (закоммичены раньше);
...
была бы лишняя точка ожидания, которой дизайн не вводит). Обязательство, для
которого локальных байтов не существует (`kill -9` в окне между commit-ом и
созданием файлов checkpoint-а), каталог незапускаемым не делает: следующий
прогон получает checkpoint текущего состояния с той mutation и доходит до
своей работы; отказ инструментом здесь — невыполненный критерий, потому что
он повторялся бы у каждого следующего прогона. Предъявлено на
...
на каждую mutation задачи — невыполненный критерий (RK-01): ожиданий у
invocation-а столько, сколько точек drain, и число mutation подкоманды его не
