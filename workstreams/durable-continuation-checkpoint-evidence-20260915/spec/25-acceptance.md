...
подразумевается: C выбрал ровно одну задачу, поэтому крашнувший вызов и
есть первая continuation-relevant mutation его прогона, acknowledged
checkpoint-а под его `run_id` нет и второе условие ключа шага 5 на нём не
выполняется (прогон, успевший закрыть задачу до краша, несёт строку
