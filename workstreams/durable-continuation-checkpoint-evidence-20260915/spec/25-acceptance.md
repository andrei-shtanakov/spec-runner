...
этим забором не накрыт и предъявляется тем же критерием с обратным знаком:
`run --all` в том же проекте выбирает задачу без ожидания, и ожидание
перед выбором — невыполненный критерий. Горячий путь `run` при этом не
