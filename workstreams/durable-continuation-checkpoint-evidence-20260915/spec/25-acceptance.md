...
отказывает `needs-human` с именем последнего `run_id`. Более поздний
...
checkpoint-а не называет), оба позже A, checkpoint-ов нет ни у одного,
snapshot A применяется; отказ здесь — невыполненный критерий, потому что
восстановить названный выходом холостой прогон нечем. Пустота **без**
closure при этом зелёным не считается: более поздний `budget authorize`,
убитый в окне между записью в DB и доставкой publisher-ом (run-start есть,
closure и checkpoint-ов нет), обязан дать `needs-human` с его `run_id`;
применённый в этой конфигурации snapshot A — невыполненный критерий, как и
на прогоне, чья closure называет неподтверждённый checkpoint. Более поздний прогон
...
каталога). Отказ на любом из трёх — невыполненный критерий.
Полнота перечня проверена подсадкой, а не чтением (знак предъявляется по
BEH-20, в `tests/test_restore_refusals.py`): подкоманда, добавленная в
`PAYING_SUBCOMMANDS` и не отнесённая ни к одной половине, красит тест
`set(PAYING_SUBCOMMANDS) == BLOCKING | NON_BLOCKING`; зелёный тест на такой
подсадке — невыполненный критерий. Задачи, растящие множество, дописывают
подкоманду в половину тем же изменением, поэтому красноты на границе задачи
быть не должно. Применённый
snapshot A в конфигурациях с open call, с блокирующим более поздним прогоном
и с более поздним прогоном без closure — невыполненный критерий.
...
config нового каталога, печатает diff, и файл виден в `git status`.
