...
закрыт, более поздний прогон C того же namespace — `spec-runner plan` в
интерактивной форме (без `--full`/`--gated`), убитый сразу после ack
call-start, — оставил open call X в `plan_agent_calls`, `restore <run_id-A>` отказывает `needs-human` с `run_id`
прогона C, `call_id` X и его provenance, `--into` остаётся пустым и `Popen`
не вызван; после закрытия X дверью restore применяется — и условие этого
названо, а не подразумевается: интерактивный `plan` до подтверждения
оператором не пишет ничего, задачи дописываются в `tasks.md` после
последнего платного вызова, поэтому ни один его call-start не идёт после
continuation-relevant mutation и acknowledged checkpoint-а под `run_id`
прогона C нет (форма существенна: `plan --full` пишет артефакт стадии
сразу после её вызова), и второе условие ключа шага 5 на нём не выполняется при том,
что ПЕРВОЕ выполняется — `plan` в блокирующей половине, то есть
конфигурация проверяет ключ, а не обходит его (дверь при этом закрывает
**call**, не прогон, и сама в блокирующую половину не входит); первый `run`
в восстановленном каталоге предъявляет open call прогона D, начатого уже
...
snapshot-а. Обратный ответ предъявлен тем же критерием на C = `run`, убитом
на той же границе: такой прогон к моменту ack call-start уже несёт
acknowledged checkpoint — флип `tasks.md` в `in_progress` есть
continuation-relevant mutation по §3 (решение владельца 2026-09-20), он
стоит до первого платного вызова, а publisher ожидается перед call-start, —
поэтому `restore <run_id-A>` обязан отказать шагом 5, назвав выходом C, и
применённый здесь snapshot A — невыполненный критерий;
...
#### AC-46: Подкоманда без платного вызова не завершается успешно, пока её checkpoint не acknowledged; `restore` не решает по store раньше доставки · verification: test
...
(механизм #527 остаётся в spec-runner#528), поэтому убитый в окне между
записью mutation и ack прогон правку теряет — это принятый пробел FR-05, и
его отсутствие критерием не является.

Второй знак того же критерия — забор на чтение у `restore` (сайт (в)
правила Q-05): в проекте с унаследованным недоставленным обязательством ack
стоит в журнале **раньше первого обращения `restore` к store** — и раньше
`list` индекса workstream-а, и раньше чтения digests последнего
acknowledged checkpoint-а, которым порядок проверок начинается, — а с
отклоняющим двойником `restore` завершается exit 2, `--into` остаётся
пустым и snapshot не применён. Любое чтение store раньше ack или
применённый snapshot — невыполненный критерий: вердикт, посчитанный по
store, который о чужой mutation не знает, теряет её молча. Выбор задачи
забором не накрыт в ветке (1) Q-12 и предъявляется тем же критерием с
обратным знаком: `run --all` в проекте с маркером `continuation_index:
local` выбирает задачу без ожидания, и ожидание перед выбором —
невыполненный критерий: недоставленным там может быть только закрытие
строки ledger-а, устаревший store отвечает «всё ещё open», и ошибка идёт в
сторону пропуска задачи, а не её повтора. Ветка (2) под область (в) подпадает, но забор в
ней пуст по построению (DB свежая, удалённая или из snapshot-а): после
`reset` тот же `run --all` идёт веткой (2) и ожидания снова не даёт —
потому что очередь пуста, а не потому, что правило его не требует. Горячий путь `run` при этом не
...
столько, сколько сайтов правила Q-05 он проходит, и число mutation
подкоманды его не увеличивает. Цену гейта перед closure меряет AC-41 (бенчмарк), не этот
критерий.
...
- **Механизмы, резолвленные design** (Q-02, Q-03, Q-05, Q-06, Q-07, Q-08,
  Q-11, Q-12): канал ack, формат WIP tar, publisher и сайты drain, модуль
