...
Чего критерий **не** требует, и это вторая половина того же разделения:
забор на чтение у `restore` (сайт (в) правила Q-05) здесь не
предъявляется. Его вход — прогон, убитый между commit-ом mutation и
доставкой её checkpoint-а, чьё обязательство подхватывает следующий
процесс, — бандлом не производится: obligation-строки и их подъём в
очередь publisher-а при первом открытии DB суть механизм
spec-runner#528, а очередь процессная и у нового процесса `restore`
пуста по построению. На пустой очереди знак «ack раньше первого
обращения к store» выполнялся бы вхолостую, а знак «с отклоняющим
двойником exit 2» недостижим — отклонять нечего. Правило записано
(Q-05, § 7.2); его наблюдаемое — критерий #528, который владеет и
механизмом, и тестом.
Горячий путь `run` при этом не
...
столько, сколько раз он совершает действия правила Q-05 — у `run` по
одному перед каждым платным вызовом плюс одно на выходе, — и число
mutation подкоманды его не увеличивает. Цену гейта перед closure меряет AC-41 (бенчмарк), не этот
критерий.
