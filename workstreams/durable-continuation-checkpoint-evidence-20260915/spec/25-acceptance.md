...
закрыт, более поздний прогон C того же namespace — `spec-runner plan`,
убитый сразу после ack call-start, — оставил open call X в
`plan_agent_calls`, `restore <run_id-A>` отказывает `needs-human` с `run_id`
...
названо, а не подразумевается: единственный платный вызов `plan`
предшествует любой его continuation-relevant mutation (задачи дописываются
в `tasks.md` после вызова), поэтому acknowledged checkpoint-а под `run_id`
прогона C нет, и второе условие ключа шага 5 на нём не выполняется при том,
