...
workstream-а **из блокирующей половины перечня** — предъявлено как минимум
на `review-pr` и на `plan` — отказывает `needs-human` с именем последнего
`run_id`; более поздний прогон из неблокирующей половины отказа не даёт, и
это предъявлено на трёх, каждый из которых оставляет свою строку в индексе
workstream-а: аудируемая дверь `evidence close-call` (в обеих
конфигурациях — где вызов жил в `agent_calls` и где в `pr_agent_calls`),
`evidence purge`, отработавший между A и восстановлением, и чужой, более
ранний `restore` — наблюдается при восстановлении A второй раз в новый
пустой `--into` (повторный запуск в занятый каталог отказывает раньше, на
проверке каталога). Отказ на любом из трёх — невыполненный критерий.
