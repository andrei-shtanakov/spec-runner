...
сторону пропуска задачи, а не её повтора. В ветке (2) обязательств нет по построению (DB
свежая, удалённая или из snapshot-а): после `reset` тот же `run --all`
идёт веткой (2) и ожидания снова не даёт — здесь потому, что требовать
нечего, а не потому, что правило не требует. Горячий путь `run` при этом не
