...
не удаляет копию, на которую ссылается строка outbox-а). Мест доставки **два**, и оба здесь
(design §3.5): рубеж старта `cli._run_start_gate` у `run`/`retry`/`watch`
(процедура Q-12 обязана читать доставленное состояние) и **первое** за
процесс обращение к `after_mutation`, которое доставляет незакрытые строки
прежде своей публикации. Второе покрывает всё остальное — `budget
authorize`, `tdd abandon|repair|resume|release`, `evidence close-call`/
`purge`, `plan`, `review-pr`, — поэтому ни одна из этих подкоманд своего
сайта ожидания не получает ни здесь, ни в DT-09/DT-12: их файлы под
доставку не правятся. Read-only команды не доставляют по построению (ни
рубежа, ни mutation) — иначе BEH-04/BEH-13 «двойник не получил ничего»
становятся ложными; `restore` исходную DB не открывает вовсе и назван
входом остатка. Наблюдаемое в этой задаче — один сайт mutation
(`record_attempt`) и старт `run`; знак на `budget authorize` и на двери —
BEH-48 (DT-05) и BEH-11 (DT-09). Вставку строки
...
`checkpoint_outbox` дверь получает **без своего сайта**: её шаг close —
mutation, а первое за процесс `after_mutation` доставляет незакрытые строки
прежде своей публикации (design §3.5, механизм — DT-03). Этой задаче
