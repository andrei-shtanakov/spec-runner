...
behaviour-спека раздала 48 сценариев по 25 целям: 23 тестовых файла (из них
...
покупается. В этом графе таких задач **нет**: ни один сценарий BEH-01…BEH-48
...
обход FR-02.
Публикация call-start идёт через тот же `Publisher` и ту же очередь, что
checkpoint-ы (design §3.5): второй синхронный путь в store мимо очереди
обгонял бы недоставленные чужие строки и тратил деньги раньше их доставки —
именно на этом порядке держится «раньше траты» у сайтов, которые открывают
state DB позже шага 1 (планирование, `review-pr`).
...
и отказные режимы **записи** closure — DT-10 (файл
`tests/test_closure_every_exit.py` его). Сам drain перед closure и его
отказный режим (гейт: closure `failed`, exit 2, строка в stderr) — DT-03,
вместе с `Publisher.drain`: владелец BEH-48 (DT-05) идёт раньше DT-10 и
обязан гасить свой красный в своих границах. Сайты выхода подкоманд ни здесь, ни
...
не удаляет копию, на которую ссылается строка outbox-а). Отдельного «сайта доставки» нет, и завести его было бы ошибкой (design
§3.5 называет обе: перечень по подкомандам забывает подкоманды, а «первое
`after_mutation`» наступает уже после своей mutation, а у `plan` — после
...
опередить их доставку — именно это, а не точка (а), держит порядок «раньше
траты» у `plan`/`review-pr`; (3) seam на шаге 1 получает открытую DB до
своего drain-а, когда DB процессу доступна вовсе. Следствия: ни одна
подкоманда своего сайта ожидания не получает ни здесь, ни в DT-09/DT-12 —
`remedy.py`, `budget_cmd.py`, `evidence_cmd.py`, `cli_plan.py`,
`review_pr.py` под доставку не правятся; read-only команды не доставляют,
потому что `Publisher`-а не создают (иначе BEH-04/BEH-13 «двойник не
получил ничего» ложны); `restore` исходную DB не открывает вовсе и назван
входом остатка. Наблюдаемое в этой задаче — один сайт mutation
(`record_attempt`) и старт `run`; знак на `budget authorize`, `plan` и на
двери — BEH-48 (DT-05) и BEH-11 (DT-09). Вставку строки
...
`excluded` — DT-05; spool в checkpoint-е и `degraded: true` — DT-08;
целостность при чтении — DT-11. Отказный режим drain перед closure —
**здесь**, вместе с самим гейтом (таймаут → closure `failed` с id
предыдущего acknowledged, exit 2, строка в stderr): владелец BEH-48 (DT-05)
обязан гасить свой красный в своих границах, а DT-10 идёт после DT-06, то
есть позже него. DT-10 остаётся правило вывода kind `closure.derive` и
остальные половины BEH-31 (повторная closure отвергнута, отказ записи
closure не улучшает код выхода, closure позже последнего ack); наблюдаемое поведение
...
BEH-48 (spec-runner#527) наблюдает гейт перед closure (точка (б)) и позднюю
доставку outbox-а на настоящих
...
поздняя доставка после `kill -9` — тремя прогонами на два события, а не
только через `run` (доставка через дверь `close-call` — знак BEH-11 в DT-09,
а её блокирующий эффект на шаг 5 — знак BEH-09 там же), — плюс контроль на `run --task`: в каталоге без недоставленных
...
видит как всякий другой checkpoint. Знак этого живёт не в BEH-48 (у его
задачи-владельца `restore`-а ещё нет), а в **BEH-09** — And про правку,
доехавшую поздней доставкой, предъявляется вместе с остальными
конфигурациями шага 5, то есть в DT-09, на поверхности уже построенного
здесь restore.
...
инвариант держит ветка (2), а не гвард на одной команде. Доставку
`checkpoint_outbox` дверь получает **без своего сайта**: незакрытые строки
встают в очередь publisher-а при открытии state DB, впереди собственных, и
дверь ждёт их на своей точке drain (design §3.5, механизм — DT-03). Этой задаче
принадлежат два **знака** поздней доставки, и оба — потому, что `restore`
здесь уже есть, а у DT-05 его не было: And BEH-11 (чужой checkpoint у
двойника раньше checkpoint-а двери и раньше её кода выхода, на
последовательности FR-05 «дверь → restore») и And BEH-09 (правка,
доехавшая поздней доставкой, блокирует шаг 5 так же, как доставленная
сразу, — единственный знак связки #527 с FR-05 на поверхности restore). Без него дверь, прошедшая мимо
...
#### DT-10: Правило вывода kind closure в диспетчере, отказные режимы записи, `kill -9` · type: implement · owner: dev
...
(гейт: `failed`, exit 2) здесь не **строятся** — их построила DT-03 (владелец
BEH-48 идёт раньше этой задачи): первый Then BEH-31 их только наблюдает.
Собственная работа этой задачи по BEH-31 — повторная closure, отказ записи
closure и порядок «closure позже последнего ack». Всё, что задача делает с kind-ами, живёт в
...
AC-11, AC-12, AC-45, AC-46 — DT-05; AC-14, AC-15, AC-16, AC-17, AC-18, AC-19, AC-39 —
...
kind-а не получают (отказ ack последнего checkpoint-а в closure при этом
отражается уже с DT-03 — гейт и его exit 2 строятся там, иначе владелец
BEH-48 не погасил бы свой красный в своих границах); между DT-03 и DT-05 строку outbox-а пишет один сайт —
