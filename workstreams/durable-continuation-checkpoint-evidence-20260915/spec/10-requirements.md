...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  код выхода и запись о завершении. Ожидание при этом **одно на
  invocation** и стоит на его выходе, а не после каждой mutation: отказ,
  поднятый между записями многошаговой подкоманды, оставил бы её решение
  применённым наполовину и незафиксированным (`tdd abandon` снимает claims
  раньше, чем пишет audit-строку), то есть покупал бы сужение окна ценой
  повреждённого состояния.
  Дополнительно mutation и обязательство её опубликовать записываются
  **одной транзакцией** (pending-outbox в state DB) — там, где mutation
  транзакцией state DB и является. Обязательство, для которого локальных
  байтов checkpoint-а не существует (процесс убит между commit-ом и их
  созданием), закрывается **снимком текущего состояния**, а не отказом:
  отказ повторялся бы у каждого следующего прогона, потому что обязательство
  снимается только доставкой, — а снимок текущего состояния ту mutation
  содержит, и для continuation обязателен только последний checkpoint; единственное исключение перечня §3 —
  harness status flip (`tasks.md` + git-коммит, соединения к DB нет):
  обязательство пишется сразу после коммита, и это не дыра, потому что сам
  флип лежит в коммите, идемпотентен по дифу и восстанавливается
  существующим механизмом (#192) — потерянный там checkpoint решения не
  теряет. Так процесс,
  убитый после commit-а, оставляет доказательство недоставленного
  checkpoint-а, а следующий прогон в этом каталоге доставляет его **раньше
  собственных** записей — и, как следствие, раньше двух событий, после
  которых прогон читают вне машины: раньше любого своего платного вызова и
  раньше собственного успешного завершения. Требование не просит «прежде
  любой другой работы» и не требует доставки раньше выбора задачи: решение
  внутри этого каталога недоставленный checkpoint не меняет — mutation,
  которую он несёт, уже в локальной DB; доставка нужна тому, кто читает
  прогон **вне** машины. Read-only команды не доставляют намеренно: от них
  требуется не отправлять в store ничего. Остаток, который этим не
  закрывается, — восстановление, начатое раньше, чем в исходном каталоге
  случилось одно из двух (в пределе — каталога и машины больше нет);
  он назван в FR-05 и fail-closed не читается. Перечень подкоманд выводится из свойства
  «решение живёт только в DB до доставки», а не из «нет платного вызова»: у
  `evidence close-call` и `evidence purge` истина уже в store, положенная
  синхронно самой подкомандой, и лишнего ожидания им не вменяется.
...
- `budget authorize` с двойником store, задерживающим ack: при превышении
  ...
  нулевой код. Строка
  authorization при этом в DB есть (она закоммичена), а недоставленный
  checkpoint уезжает на следующем прогоне в этом каталоге — раньше его
  собственных записей, то есть раньше его платного вызова и раньше его
  успешного выхода. То же — для `tdd release` и для многошагового `tdd
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  завершается успешно до ack своего mutation-checkpoint-а, обязательство
  опубликовать
  пишется той же транзакцией, что mutation, и следующий прогон в каталоге
  доставляет недоставленное раньше **собственных** записей — то есть, в
  формулировке FR-03, раньше своего платного вызова и раньше успешного
  завершения; более сильного «прежде любой работы» требование не
  просит ни здесь, ни там. Остаётся
  единственный необслуживаемый остаток: в исходном каталоге после смерти
  прогона не отработал ни один **доставляющий** прогон. Доставляющий — тот,
  кто открыл канал публикации и дождался его хотя бы раз: read-only команды
  (`status`, `costs`, `validate`, `report`, `evidence <run_id>`) канала не
  открывают вовсе, поэтому каталог, где после аварии запускали только их,
  остаток **не** закрывает — успешно завершившийся `status` доставки не
  гарантирует; `restore`, запущенный в самом пострадавшем каталоге,
  доставляет, а запущенный на другой машине — нет, потому что того каталога
  там нет. В пределе каталога и машины больше нет, и pending-outbox уехал
  вместе с ними. Недоставка чужой правки при этом не делает прогон
  неуспешным: exit 2 требуется за **свой** недоставленный checkpoint, иначе
  одна авария роняла бы каждую следующую команду в каталоге, включая
  `restore`, которым из неё выходят.
...
- **Адрес store не зависит от рабочего каталога процесса.** Путеподобные
  настройки адаптера store резолвятся в абсолютные при загрузке config —
  один раз, против `project_root`, — потому что процесс вправе сменить CWD
