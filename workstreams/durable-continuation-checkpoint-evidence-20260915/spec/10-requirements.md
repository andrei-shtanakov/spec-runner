...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  **одной транзакцией** (pending-outbox в state DB), так что процесс,
  убитый после commit-а, оставляет доказательство недоставленного
  checkpoint-а, а следующий прогон в этом каталоге доставляет его прежде
  своей работы — на старте, если это прогон задач, иначе перед первой
  собственной mutation. Read-only команды не доставляют намеренно: от них
  требуется не отправлять в store ничего, и ни старта прогона задач, ни
  mutation у них нет. Остаток, который этим не закрывается, —
  восстановление, начатое раньше, чем в исходном каталоге случилось
  то или другое (в пределе — каталога и машины больше нет); он назван в
  FR-05 и fail-closed не читается. Перечень подкоманд выводится из свойства
...
- `budget authorize` с двойником store, задерживающим ack: успех не
  ...
  authorization при этом в DB есть (она закоммичена), и рядом с ней —
  запись pending-outbox, которую следующий invocation в этом каталоге
  доставляет прежде любой другой работы. То же — для `tdd release`.
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  прогона не случилось ни старта прогона задач, ни чужой mutation — то есть
  доставить недоставленное было некому, — в пределе потому,
  что каталога и машины больше нет и pending-outbox уехал вместе с ними.
