...
approved_at: '2026-09-20T15:06:47Z'
---
...
- **Платный subprocess (paid call)** — запуск CLI провайдера с prompt-ом.
  ...
  платный вызов), `review-pr` verify и fix, и **два** сайта пробы `doctor` —
  исполнение канонной задачи и, под `--with-review`, её review. Бриф в FR-06 называет
...
- **Provenance / stage** — существующее поле `provenance` (`red`, `red:fix`
  — RED agent round #220, `green`,
  ...
  `doctor` (`doctor:execute`, `doctor:review`). Одно поле, один словарь.
  У `doctor` **два** значения, а не одно: у пробы два платных сайта, и
  единственное значение `doctor` не различало бы в ledger-е и в evidence
  стоимость исполнения от стоимости review — ровно того, что `doctor
  --with-review` и предъявляет оператору. Ни одно значение словаря не
  остаётся без сайта, который его производит (FR-02).
...
- **Continuation-relevant mutation** — запись, без которой следующий шаг
  ...
  `waivers_applied`, `tdd_remedies`, `budget_authorizations`, `pr_*`,
  **закрытие строки call-ledger-а любого семейства** (`agent_calls`,
  `pr_agent_calls`, `plan_agent_calls`) — именно оно отличает «вызов
  оплачен и закрыт» от open call, то есть решает, повторит ли следующий шаг
  оплаченный вызов (FR-02), — а также
...
- Ledger-таблицы `attempts`, `agent_calls`, `pr_agent_calls` и
  `plan_agent_calls` (FR-06: третья таблица семьи для вызовов без задачи)
  получают
...
- `call_id` пишется рядом с существующим `provenance` (FR-01) в ledger
  **того семейства, к которому относится вызов** — `agent_calls` (вызов
  задачи), `pr_agent_calls` (`review-pr`), `plan_agent_calls` (планирование,
  FR-06), — так что ledger стоимости и evidence соединяются одним ключом
  для каждого сайта без исключения.
- **Область пробы `doctor`.** Платный вызов `doctor` исполняется в
  эфемерном scratch-проекте, который удаляется вместе со своим state DB, и
  отсюда разделение двух областей, а не одна. **Внешний invocation**
  владеет всем, что обязано пережить прогон: `run_id`, run-start, обе записи
  каждого платного вызова, стоимость и closure — деньги настоящие, и запись
  о них не эфемерна. **Внутренняя проба** владеет только своим scratch:
  её ledger-строки, attempt и статусы живут в удаляемой DB и
  continuation-relevant mutation (§3) не являются — продолжать снимок
  удалённого каталога нечего (FR-03, FR-06). Следствия, каждое —
  требование: (а) опубликованная запись вызова пробы не несёт `task_id`
  проекта (канонная задача пробы адресуема только внутри scratch, и
  восстановление такой строки в чужой DB заблокировало бы **одноимённую
  реальную** задачу проекта — FR-02 namespace-wide правило не про неё);
  (б) `task_id` нужен nullable только опубликованной записи, чью схему
  вводит этот бандл, — столбец `agent_calls.task_id` остаётся `NOT NULL` и
  миграции не требует, потому что строку пробы принимает scratch-DB, где
  канонная задача настоящая; (в) сайтов у пробы ровно два, и это
  обеспечено, а не предположено: scratch-конфиг пробы объявляет
  `execution_mode: standard`, иначе проект под `tdd` дал бы пробе третий
  платный вызов (RED authoring) — больше, чем cost gate `doctor` объявил
  оператору.
...
- `doctor --with-review` с fake CLI: два платных вызова, provenance
  `doctor:execute` и `doctor:review`, у каждого пара call-start/call-result;
  ни одна опубликованная запись не несёт `task_id`, а под `run_id` этого
  invocation нет ни одного checkpoint-а и ни одного attempt-экспорта.
  Следующий `run --all` проекта, у которого есть своя `TASK-001`, берёт её,
  а не находит заблокированной пробой.
...
- **Точки публикации** — сайты записи перечисленных таблиц и сайты записи
  harness-флипов `tasks.md`: не только `bookkeeping.commit_status_flip`
  (он коммитит флип, а не публикует его), но и сайты самой записи —
  `in_progress` и `review`, которые harness пишет о собственном процессе
  до платного вызова (решение владельца 2026-09-20 о широком прочтении
  §3; перечень сайтов — design §3.1). Одна функция «после mutation»,
  вызываемая из них, а не разбросанные вызовы (по образцу
  `run_plugin_hooks_for`, #307).
...
- **Эфемерная проба checkpoint-ов не публикует.** Запись в state DB
  scratch-проекта `doctor` (FR-02, «область пробы») continuation-relevant
  mutation не является: DB удаляется вместе с каталогом, а manifest такого
  снимка нёс бы identity временного каталога. Проба поэтому не публикует ни
  checkpoint-ов, ни attempt-экспорта (FR-06) под `run_id` вызвавшего её
  invocation; платные вызовы пробы публикуются полностью (FR-02).
  Наблюдаемое следствие для FR-05: прогон `doctor` не бывает ни выходом,
  который отказ restore называет оператору, ни блокирующим прогоном —
  acknowledged checkpoint-а у него нет ни одного, по построению, а не по
  строке перечня.
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  У `budget authorize` и `tdd abandon|repair|resume|release` платного вызова
  нет, поэтому точка ожидания доставки «перед call-start» у них не
  наступает, и между записью mutation и доставкой checkpoint-а остаётся
  окно, в котором убитый процесс теряет своё решение молча (FR-05,
  принятый пробел). Требование сужает окно, а не меняет правило чтения:
  такая подкоманда **не завершается успешно**, пока checkpoint её mutation
  не acknowledged; таймаут ack — отказ инструмента (exit 2), не успех, а
  напечатанная раньше строка результата успехом не считается — авторитетны
  код выхода и запись о завершении. Ожидания при этом стоят **на границах
  invocation-а**, а не после каждой mutation: отказ, поднятый между
  записями многошаговой подкоманды, оставил бы её решение применённым
  наполовину и незафиксированным (`tdd abandon` снимает claims раньше, чем
  пишет audit-строку), то есть покупал бы сужение окна ценой повреждённого
  состояния. Сколько границ проходит invocation — зависит от того, какие
  действия он совершает (решение владельца 2026-09-20, Q-05 в дизайне): у
  `budget authorize` и `tdd …` это одна граница — выход; у прогона с
  платным вызовом добавляется граница перед ним. Правило называет и третью
  — перед первым чтением store у `restore`, потому что решение, принятое
  по недоставленному состоянию, тихо теряет чужую правку, — но **эта
  волна её не строит**: ждать там нечего, пока нет самих обязательств, и
  граница приезжает вместе с ними (spec-runner#528). То есть требование
  фиксирует, где ожидание обязано стоять, а не сколько их наступает
  сегодня. Неизменным остаётся
  то, ради чего формулировка и писалась: синхронного ожидания внутри
  `after_mutation` нет, поэтому число ожиданий не растёт с числом mutation,
  а у подкоманды без платного вызова оно одно и на выходе. Чего это НЕ
  утверждает: у invocation-а с двумя и более платными вызовами ожидание
  перед вторым из них стоит после записей, сделанных вокруг первого, —
  и это допустимо по другой причине, чем у отвергнутой точки «сразу после
  mutation»: отказ там наступает **до spawn**, то есть до траты, и
  оставляет за собой только завершённые записи, тогда как ожидание внутри
  `after_mutation` рвало бы одну логическую операцию между её записями
  (`tdd abandon` снимает claims раньше, чем пишет audit-строку).
  Окно этим сужается, но не устраняется: `kill -9` внутри него по-прежнему
  теряет правку, и остаток остаётся принятым (FR-05). Третье, что называет
  заявка #527, — «полезное усиление, transactional pending-outbox в SQLite»,
  — это требование **не вводит**: обязательство публикации, записанное
  транзакцией mutation, тянет за собой правила для manifest-а, порядка
  проверок restore, области эфемерной пробы и сайтов, транзакцией state DB
  не являющихся (harness status flip). Граница ожидания, которой ему не
  хватало на пути «crash → restore», в бандле теперь есть — правило Q-05
  выше называет её среди своих (решение владельца 2026-09-20); вынесен
  сам механизм: obligation-строки в транзакции mutation, их подъём в
  очередь при первом открытии DB, правило для обязательства без локальных
  байтов и снятие унаследованных обязательств при `restore`. Объём —
  отдельной задачей (spec-runner#528) со всем, что о нём установлено.
  Кому гейт меняет исход: `budget authorize` и
  `tdd abandon|repair|resume|release` держат решение **только** в DB до
  доставки, поэтому для них exit 2 при недоставке и есть содержание заявки;
  у `evidence close-call`/`purge` истина уже в store, положенная синхронно
  ими самими, — гейт стоит и у них, но исхода не меняет.
...
- `budget authorize` с двойником store, задерживающим ack: при превышении
  таймаута — exit 2 и сообщение, называющее недоставленный `sequence`, а не
  нулевой код. Строка authorization при этом в DB есть (она закоммичена), и
  больше ничего не обещано: недоставленный checkpoint так и остаётся
  недоставленным — это принятый остаток FR-05, а поздняя доставка вынесена
  в spec-runner#528. То же — для `tdd release` и для многошагового `tdd
  abandon`, у которого под тем же двойником в DB есть **все** его записи,
  включая audit-строку, а не только первые.
- `doctor --with-review` с fake CLI: двойник store не получил ни одного
  checkpoint-а и ни одного attempt-экспорта, при этом обе пары
  call-start/call-result получил.
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  блокирует более поздний, выходом не называется — это была бы петля. Такой прогон существует всегда, когда отказ
  сработал: блокирующим прогон делает в том числе acknowledged checkpoint,
  а самый поздний из блокирующих не имеет блокирующих после себя. Наблюдаемым признаком «оставил» служит
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  без manifest-а таковым не считается — design § 1.1); отсюда **принятый пробел** (решение
  ...
  «закрыть open call → восстановиться» не работал бы ни разу. Поэтому
  предикат остаётся тем же, а **окно сужается** (решение владельца
  2026-09-18, вторая половина): подкоманда без платного вызова не
  завершается успешно до ack своего mutation-checkpoint-а, и окно потери
  сводится к хвосту её handler-а вместо всего времени до завершения
  прогона. Убитый внутри этого окна прогон правку по-прежнему теряет —
  остаток принят и назван; транзакционное обязательство публикации, которое
  закрыло бы его на живой машине, вынесено отдельной задачей
  (spec-runner#528) вместе со всем, что о нём установлено. Требование
  «доставка раньше, чем `restore` читает store» вынесенным больше НЕ
  считается: это граница ожидания, и она названа правилом Q-05 (решение
  владельца 2026-09-20). За #528 остаются сами обязательства, носитель,
  которым они попадают в очередь `restore`, правило для обязательства без
  локальных байтов и снятие унаследованных.
  Прогон, ничего такого
...
- **Планирование** получает ledger-identity: `plan --full` и `plan --gated`
  ...
  `plan:interactive`, все — **без задачи**;
  `costs` показывает их отдельной строкой «planning», не смешивая с task
  cost (по образцу `pr_cost_rows`, #218). Строка вызова без задачи ложится в
  **свой ledger той же семьи**, а не в `agent_calls` со значением `NULL`:
  так уже решён тот же вопрос для `review-pr` (`pr_agent_calls` заведена
  «третьей таблицей вместо nullable `task_id`» именно потому, что `costs`
  группирует task-ledger по задаче и строку без задачи пришлось бы
  особить каждому читателю той поверхности — `docs/state-schema.md`,
  #218). Отсюда: `agent_calls.task_id` остаётся `NOT NULL`, смены типа
  столбца бандл не объявляет, и ни одна миграция не делает существующую
  колонку nullable.
...
- **Адрес store не зависит от рабочего каталога процесса.** Путеподобные
  настройки адаптера store разрешаются в абсолютные пути при загрузке
  config — один раз и относительно `project_root`, — потому что процесс
  вправе сменить CWD
  внутри себя (`doctor` уходит в scratch и удаляет его), и относительный
  адрес, разрешённый после этого, дал бы платный вызов без единой durable
  записи: каталог с записями исчез бы вместе с пробой.
- **Attempt-экспорт эфемерной пробы не публикуется** (FR-03): он назвал бы
  задачу, которой в workstream-е нет, и корроборировал бы checkpoint,
  которого нет. Стоимость пробы при этом не теряется — она в call-result
  каждого её вызова.
