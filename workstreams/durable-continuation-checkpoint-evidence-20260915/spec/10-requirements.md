...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  **одной транзакцией** (pending-outbox в state DB), так что процесс,
  ...
  checkpoint-а, а следующий invocation в этом каталоге доставляет его
  прежде любой другой работы. Остаток, который этим не закрывается, —
  восстановление, начатое раньше, чем в исходном каталоге снова что-либо
  запустили (в пределе — каталога и машины больше нет); он назван в FR-05 и
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  единственный необслуживаемый остаток: восстановление начато раньше, чем в
  исходном каталоге снова что-либо запустили, — в пределе потому, что
