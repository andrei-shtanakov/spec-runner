...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  **одной транзакцией** (pending-outbox в state DB) — там, где mutation
  ...
  собственных** записей — и, как следствие, раньше трёх событий, на которых
  состояние читают: раньше любого своего платного вызова, раньше выбора
  задачи (если это прогон задач) и раньше собственного успешного
  завершения. Требование не просит «прежде любой другой работы»: важно не
  то, что процесс ничего не делает до доставки, а то, что он не **решает**
  и не тратит. Read-only команды не доставляют намеренно: от них
  ...
  случилось хоть одно из трёх (в пределе — каталога и машины больше нет);
...
- `budget authorize` с двойником store, задерживающим ack: успех не
  ...
  собственных записей, то есть раньше его платного вызова, выбора задачи и
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  формулировке FR-03, раньше своего платного вызова, выбора задачи и
  успешного завершения; более сильного «прежде любой работы» требование не
  просит ни здесь, ни там. Остаётся
  единственный необслуживаемый остаток: в исходном каталоге после смерти
  прогона ни один прогон не дошёл ни до платного вызова, ни до выбора
  задачи, ни до своего завершения — то есть доставить недоставленное было
