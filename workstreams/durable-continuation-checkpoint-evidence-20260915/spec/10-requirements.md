...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  такая подкоманда **не сообщает об успехе**, пока checkpoint её mutation
  не acknowledged; таймаут ack — отказ инструмента (exit 2), не успех.
  ...
  **одной транзакцией** (pending-outbox в state DB) — там, где mutation
  транзакцией state DB и является; единственное исключение перечня §3 —
...
- `budget authorize` с двойником store, задерживающим ack: успех не
  напечатан раньше ack; при превышении таймаута — exit 2 и сообщение,
  называющее недоставленный `sequence`, а не нулевой код. Строка
  ...
  успешного выхода. То же — для `tdd release`.
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  2026-09-18, вторая половина): подкоманда без платного вызова не сообщает
  об успехе до ack своего mutation-checkpoint-а, обязательство опубликовать
