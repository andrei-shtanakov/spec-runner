...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  прогона ни один **доставляющий** прогон не дошёл ни до платного вызова,
  ни до своего завершения. Доставляющий — тот, кто вообще открывает store
  для записи: read-only команды (`status`, `costs`, `validate`, `report`,
  `evidence <run_id>`) и сам `restore` не доставляют по построению, поэтому
  каталог, где после аварии запускали только их, остаток **не** закрывает —
  успешно завершившийся `status` доставки не гарантирует. В пределе
  каталога и машины больше нет, и pending-outbox уехал вместе с ними.
