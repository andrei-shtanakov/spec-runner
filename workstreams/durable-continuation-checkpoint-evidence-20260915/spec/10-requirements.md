...
- **Синхронный ack mutation-checkpoint-а у подкоманд без платного вызова.**
  ...
  **одной транзакцией** (pending-outbox в state DB), так что процесс,
  ...
  checkpoint-а, а следующая подкоманда, которая в этом каталоге открывает
  DB **и** вправе публиковать, доставляет его прежде любой другой работы.
  Read-only команды не доставляют намеренно: от них требуется не отправлять
  в store ничего. Остаток, который этим не закрывается, —
  восстановление, начатое раньше, чем в исходном каталоге отработала такая
  подкоманда (в пределе — каталога и машины больше нет); он назван в FR-05 и
  fail-closed не читается. Перечень подкоманд выводится из свойства
...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  прогона не отработала ни одна подкоманда, которая доставляет
  недоставленное (открывает DB и вправе публиковать), — в пределе потому,
