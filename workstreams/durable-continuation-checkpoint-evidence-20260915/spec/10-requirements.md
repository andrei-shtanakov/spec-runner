...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  **acknowledged** checkpoint прогона (частично доставленный набор ключей
  ...
  (spec-runner#528) вместе со всем, что о нём установлено, включая то, что
  на пути «crash → restore» оно требует доставки раньше, чем `restore`
  читает store в своих проверках.
