...
- **Порядок проверок (все — до записи в каталог):** contract version →
  ...
  называет как выход последний прогон workstream-а, у которого есть
  acknowledged checkpoint (не просто последний по времени: собственная запись
  текущего `restore` и прогоны без checkpoint-а невосстановимы), а если
  такого прогона нет — говорит об этом и выхода не предлагает. Прогон, ничего такого
