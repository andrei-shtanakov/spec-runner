"""Contract test for BEH-10 (spec-runner#341, TASK-008).
...
class TestCompositeLintCommandDeclaresFixSkipped:
    ...
    def test_composite_lint_command_with_no_fix_declared_also_names_the_reason(
        self, tmp_path_factory, monkeypatch
    ):
        """Composite is composite whether or not a fix invocation is
        declared — the reason named must stay the composite-specific one,
        not the "fix invocation was never declared" wording used when the
        check command is a plain, single command."""
        scripts = tmp_path_factory.mktemp("scripts")
        check_script = scripts / "check_lint.py"
        check_script.write_text(_ALWAYS_FAILING_CHECK_SCRIPT)

        root = _repo(tmp_path_factory.mktemp("composite_nofix"))
        composite_check = f"{_shell_command(check_script)} && true"
