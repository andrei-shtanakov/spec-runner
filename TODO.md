...
- 🚀 **`[Unreleased]`: четыре PR после 2.29.0.** #226 (#218 этап 1 — собственный
  ...
  Публичной поверхности не трогают: ни новых ключей схем, ни флагов. Кандидат —
  патч/минор по составу на момент выпуска.
...
- 🔎 **Открыты из каскада:** #230 (инфра-попытки съедают бюджет задачи; решение
  за владельцем — отдельный infra-бюджет / возврат / авторизованное расширение)
  и #232 (после `tdd repair` чекпойнт честно `not_red`, потому что работа
