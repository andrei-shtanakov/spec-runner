...
- [ ] **runtime-state-artifact-export** (spec-runner#480) @owner:TBD @id:runtime-state-artifact-export
      ...
      **Следствие, которое надо проверить до платных прогонов:** наша
      tasks-спека #480 авторена под СТАРЫЙ диалект и `delivers` не несёт.
      Прежде чем возобновлять, нужно либо переавторить её под
      `dt_contract_version: 2`, либо убедиться замером, что мост принимает
      старую. Возобновление платных прогонов — решение владельца; здесь
      зафиксирован только снятый блокер, а не разрешение тратить.
