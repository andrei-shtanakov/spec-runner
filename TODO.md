...
- [ ] **beh-09-close-call-scenario-unreachable** (spec-runner#480, из ревью PR #522) @owner:TBD @id:beh-09-close-call-scenario-unreachable
      ...
      **Не закрыт: упирается в решение ниже.** Перевывод снял механизм
      находки, но достижимость конфигурации C зависит от того, публикует
      ли checkpoint флип `in_progress`. При широком прочтении §3 любой
      `run`, дошедший до ack call-start, уже несёт доставленный checkpoint
      (ожидание publisher-а стоит перед call-start), и ветка «restore
      применяется» недостижима ровно по исходной находке. BEH-09 и AC-08
      написаны против узкого прочтения и называют это условием, а не
      утверждением о коде.
      ...
      **Решение владельца, которого не хватает — расхождение §3 и §3.1 по
      ...
      через один из них. То есть §3.1 описывает доставленный код, а §3
      шире него. Варианта два: сузить §3 до флипов, идущих через
      `bookkeeping.commit_status_flip` (тогда объявленный факт C
      обоснован, и пункт выше закрывается), либо признать флип
      `in_progress` публикуемым (тогда переписывать конфигурацию C —
      неплатящей подкомандой, а не `run`). Это решение владельца о
      семантике требований, не правка формулировки.
