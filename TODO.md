...
- [ ] **bundle-480-narrow-checkpoint-delivery-window** (spec-runner#527, from devtools) @owner:github:andrei-shtanakov @id:bundle-480-narrow-checkpoint-delivery-window
      ...
      успешного завершения; на живой машине потеря становится поздней
      доставкой. Доставки раньше **выбора** задачи не требуется: решение
      внутри каталога недоставленный checkpoint не меняет, mutation уже в
      локальной DB.
      Необслуживаемый остаток назван прямо: в исходном каталоге после смерти
      прогона не отработал ни один **доставляющий** прогон — тот, кто открыл
      канал публикации и дождался его хотя бы раз (read-only команды его не
      открывают, так что успешный `status` доставки не гарантирует;
      `restore` в самом пострадавшем каталоге доставляет, на другой машине —
      нет; в пределе — каталога и машины больше нет).
