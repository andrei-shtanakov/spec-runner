...
- 🔎 **Открытые после этого круга:** #218 (решение владельца: этап 1 — починить
  собственный лимит `review-pr`, считать оба вида вызовов и проверять перед
  каждым; этап 2 — отдельная таблица `pr_agent_calls`, не nullable `task_id`),
  #220 (RED-фаза линтует дефолтным `uv run ruff check .`, поэтому на проекте
  без объявленного линтера TDD не работает вовсе, а `unverifiable` превращает
  это в платные повторы; решение владельца — композиция с четырьмя границами).
