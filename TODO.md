...
**Дрейф `upstream_hashes` внутри бандла оставлен намеренно.** Правки
2026-09-20 сдвинули байты `15-behaviour-spec.md` и `25-acceptance.md`, а
`20-design.md`/`25-acceptance.md`/`30-decomposition.md` продолжают пинить
их пре-правочные блобы. Это не забытый хвост: по DEC-008 пин ставит
**только** `spec approve` (`stamp_authoring_links(..., pin_upstream=True)`
зовётся из `apply_approval` и больше ниоткуда), а ручная перепиновка
черновика — ровно тот дефект, который заведён как spec-runner#426. Узлы
не одобрены, каскад `stale` статусом не срабатывает, и пины встанут на
место при первом же approve цепочки.
