...
- [ ] **bundle-480-narrow-checkpoint-delivery-window** (spec-runner#527, from devtools) @owner:github:andrei-shtanakov @id:bundle-480-narrow-checkpoint-delivery-window
      ...
      **Развилка #528 снята 2026-09-20 (решение владельца): правило Q-05
      ...
      Правки: §Q-05 (формулировка + область + пересчёт отказа), §3.5,
      §7.2 (шаг 0, перечень instrument-исходов, абзац про вынесенный
      механизм), §3.1, карта модулей (`checkpoint.py` — оба отказных
      режима drain; `restore_cmd.py` — шаг 0 и BEH-48; `remedy.py`), DT-03
      в decomposition, BEH-48 и AC-46 (три новых знака: порядок ack против
      первого `list`, exit 2 без применения snapshot-а, и невмешательство
      в выбор задачи).
