...
- 🚀 **`[Unreleased]` снова не пуст: два PR после 2.29.0.** #226 (#218 этап 1 —
  ...
  **перед** каждым; `verify_comment` наконец знает свою стоимость; неположительный
  ...
  линтер, плюс отказ RED больше не теряет причину). Публичной поверхности не
  трогают: ни новых ключей схем, ни флагов; изменилось только поведение
  необъявленного дефолта. Кандидат — патч/минор по составу на момент выпуска.
