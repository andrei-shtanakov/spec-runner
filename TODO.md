...
- ✅ **v3.0.0 выпущен и отрепетирован 2026-09-22** (PR #570, merge `b51f0b4`;
  тег на **merge-коммите** по runbook §2 — правило одно и закрыто решением владельца 2026-09-22: тег ставится на
  merge-коммит по runbook §2; деревья `fc38ae3` и `b51f0b4` идентичны, tree
  `79e4769`). **Major по правилу, не по поломке:** против v2.36.0
...
- ✅ **#428 закрыт 2026-09-22 — исполняемый негативный контроль для
  ...
  `**Negative-control:** <путь> :: <селектор>` — патч из своего кандидат-коммита,
  ломающий проверяемое свойство; харнесс реплеит обе половины до платного
  ревью, до ревьюера доезжает только satisfied. Свидетельство — таблица
  `negative_controls` (`docs/state-schema.md`), **следующий релиз — major**
  (AP-12.3, объявлено в CHANGELOG; bump — ритуал релиза). Дормантность
  измерена дифференциально. Мост devtools уведомлён — devtools#336, правок
  у соседа нет. **DT-06** (контроль по требованию, Could) — не начат, по
  решению владельца. Приёмка #565 дала две настоящие блокирующие (контроль
  судит только коммит, несущий работу; нечитаемое дерево ≠ чистое) — обе
  про границу коммит/рабочее дерево, куда 18 локальных кругов не смотрели;
  правило остановки локального цикла заведено постфактум. **DT-06 (Could)
  отгружен, не вырезан** — PR #568 `5c07a8a`: `spec-runner tdd control
  <TASK-ID> [--sha] [--json]`, судья один с гейтом, свидетельства не пишет
  (BEH-22); приёмка — approve без находок. Граф DT-01…DT-06 закрыт целиком.
...
- ✅ **v2.35.0 выпущен и отрепетирован 2026-08-21** — одна точка расширения
  ...
  `docs/state-schema.md`, строки `add_argument`) пуст — ни флаг, ни exit-код, ни
  колонка, ни ключ `--json-result` не двигаются, и манифест плагина, который
  новую точку не называет, видит поведение v2.34.0. Двигается словарь,
  доступный манифесту: опубликованная точка расширения — обещание, на которое
  пинуются потребители, и patch был бы правдой про существующих вызывающих и
  неправдой про контракт (PR #310).

  Поверхности измерены: тег на релизном коммите `ee88794` (не на merge
  `923ec14`; с v3.0.0 правило — тег на merge-коммит по runbook §2), `publish.yml` (`32492421127`) success, PyPI 200
  на per-version endpoint и `info.version` = 2.35.0, GitHub Release создан из
  секции CHANGELOG, красный `release-tag-guard` (`32492347227`) перезапущен
  зелёным, установка `--refresh --reinstall --no-cache` с утверждением
  напечатанной версии (`2.34.0` → `2.35.0`, то есть подмена бинаря состоялась).

  **Репетиция опубликованного артефакта — PASS.** `tests/test_post_review_plugin_edge.py`
  прогнан против **колеса с PyPI**, а не против чекаута (`uv run --no-project
  --with spec-runner==2.35.0`, импорт сверен по `spec_runner.__file__` из
  `~/.cache/uv/archive-*`): 18 passed. Стенд проверен на различимость
  контрольным прогоном тех же тестов против `spec-runner==2.34.0` — **11 failed,
  7 passed**. Зелёное, которое не краснеет на предыдущем релизе, ничего не
  доказывало бы об этом.

  Релиз снимает триггер у соседа: disputatio ждал `post_review` в выпущенной
  версии для `tdd-gate-migration-proof`.

- ✅ **v2.34.0 выпущен и отрепетирован 2026-08-19** — обе половины #301 плюс
  ранее сделанные #295/#296. **Minor, а не patch**, и это решение, а не
  формальность: дифф против v2.33.2 по трём поверхностям runbook'а
  (`schemas/`, `docs/state-schema.md`, строки `add_argument`) непустой —
  расширен enum `attempts.error_kind`, и поле, бывшее NULL на большинстве
  отказов, теперь несёт значение. Потребитель с пиненой копией старой схемы
  (Maestro вендорит нашу) отверг бы строки, которые пишет сам spec-runner.
  Тот enum был неверен и до релиза (код с v2.3.0 писал `blocked`/`api_error`),
  так что major не нужен: ранее невалидные строки становятся валидными.

  Поверхности измерены: тег на релизном коммите `8e4a65e` (не на merge; с
  v3.0.0 правило — тег на merge-коммит по runbook §2), `publish.yml` success, PyPI 200 на per-version endpoint и
  `info.version` = 2.34.0, GitHub Release, красный `release-tag-guard`
  (`32252642865`) перезапущен зелёным, установка `--no-cache` с утверждением
  напечатанной версии.

  **Репетиция опубликованного артефакта — PASS**, на 2.34.0, с недоступными
  из PATH `claude`/`codex` (фолбэк на реального агента упал бы на exec, а не
  потратил деньги). Расход — 0. Прогон под TDD-гейтом: exit 1, `attempts` с
  `error_kind=policy` / `error_stage=tests`, ERROR-запись `Task failed` в
  jsonl со всеми полями, `status` печатает причину целиком с `[policy]` и
  `[at: tests]`, вызов `status` не создаёт ни одного файла (пустых jsonl — 0).
  Второй прогон воспроизвёл **дословно сценарий из issue** (red написан в уже
  существующий файл) — причина длиной 340 символов доехала до stderr целиком,
  включая финальное «write the failing test in a file of its own».
  Плановая половина проверена импортом из опубликованного колеса
  (`importlib.metadata.version` = 2.34.0): отчётный файл как есть — 2 ошибки,
  после нормализации — 2 задачи с сохранённым графом зависимостей,
  неисправимая мета — отказ калитки словами валидатора и exit 1.

  **Замеренное ограничение:** `--json-result` режет `error` до 200 символов
  (пинается схемой `json-result.schema.json`), поэтому в этом канале хвост
  сообщения теряется; полный текст — на stderr, в jsonl, в `attempts` и в
  `status`. Расширять — отдельное решение по контракту.
- ✅ **v2.33.2 выпущен и отрепетирован 2026-08-14** — один фикс: авторизация
  применяется целиком или никак (#289). Выпущен **до** любых новых авторизаций
  по решению владельца: без него неудачная dual-axis команда оставляет
  половинное финансовое решение, а платный прогон нельзя строить на версии с
  известной неатомарностью authority-операции.

  Поверхности измерены: тег на релизном коммите `a4ca060`, `publish.yml`
  success, PyPI 200 и `info.version` = 2.33.2, GitHub Release, красный
  `release-tag-guard` перезапущен зелёным, установка `--no-cache` с
  утверждением напечатанной версии.

  **Репетиция опубликованного артефакта — PASS**, вся заданная владельцем
  последовательность на временных доменах, без агента и без касания kapelle:
  устаревший run `--after` не двигает task-ось, устаревший task `--after` не
  двигает run-ось, отказ монотонности не пишет ни строки, чистая dual-axis
  авторизация применяет обе оси и показывает резерв, повтор отказывает
  идемпотентно и оставляет ровно первое решение, эффективный потолок совпадает
  с авторизованным. Расход — 0.
- ✅ **Третья evidence-задача выполнена: S4 needs-human hold path** (kapelle
  m2 TASK-105), запись —
  `docs/superpowers/specs/2026-08-14-tdd-run-task-105-needs-human-hold.md`.
  Авторизации #10 (task, $4.00) и #11 (run, $30.95), обе с резервом
  `review=1.25`, обе абсолютные и посчитанные как «расход на момент решения +
  $4.00 новых денег». Факт: **$4.1643** (RED $2.6944 + GREEN $1.4700, ревью
  $0.00), домен $26.95 → $31.11.

  Product-вердикт — чисто с первой попытки: 514 s, обе терминальные калитки
  satisfied дважды, 444 теста без падений, golden-оракул только читается.
  Никаких remedy, никаких отказов, lifecycle прошёл целиком. Оракул
  `golden/needs_human` был подготовлен **бесплатно до денег** (kapelle PR #18).
  Работа влита в integration-ветку, master не тронут, kapelle PR #19 открыт —
  мержит человек.

  Впервые в реальном прогоне отработали: резерв #267 (GREEN видел $2.75,
  ревью — $4.00), объявление перерасхода #255 part 1 (`$31.11 > $30.95` в
  stderr, «работа стоит, новых платных вызовов не будет»), освобождение
  claims на DONE #260 и `integration_pr` #254.
- 🆕 **Два новых дефекта наблюдаемости, найдены артефактами самого прогона
  TASK-105** (оба — в логировании промптов, отгруженном в 2.33.1):
  - **#295** — RED-артефакт пишет вопрос и не пишет ответ: `tdd._log_prompt`
    зовёт `log_prompt` и никогда `append_output`, тогда как green и оба
    review-пути дописывают. Самый дорогой вызов задачи ($2.6944, 27 520
    output-токенов) не оставил следа о том, что произвёл, — ровно то, ради
    чего заводился #282. Мой же rehearsal 2.33.1 проверял `=== OUTPUT ===` у
    green и review, а у RED — только наличие текста промпта, и прошёл вхолостую.
  - **#296** — артефакт отказанного вызова неотличим от упавшего: промпт
    пишется в `review.py:539`, бюджетная калитка живёт внутри `_run_reviewer`
    (547). Ревью отказали до старта, файл на 6881 B содержит полный промпт и
    ни output, ни return code, ни cost — та же байтовая форма, что у
    процесса, который стартовал и умер. Лечится терминальной секцией
    `=== NOT STARTED: <reason> ===`, а не переносом записи за калитку.
- 📊 **Operational-счётчик #285 — 3/3, триггер НЕ сработал.** Три evidence-записи
  написаны бесплатно из durable-артефактов канонического домена kapelle m2
  (`docs/superpowers/specs/2026-08-14-tdd-run-task-101-*.md` и `…-task-104-*.md`):
  обе задачи настоящие продуктовые, обе дошли до подтверждённого red с
  чекпойнтом и byte-lock, обе прошли терминальные гейты и человеческий мерж.

  Классы post-GREEN **разные и оба не рефакторинговые**: у TASK-101 в коде
  ничего, а единственная находка — тесты green'а пришлось вынести из
  заклеймленного файла — стала #252 и уже отгружена; у TASK-104 настоящий баг
  нашёл человек **после** мержа (`59fe660`, rejection reasons не JSON-safe перед
  `jsonb`), но ревью там **не запускалось вовсе** — бюджет кончился до него.
  По таблице чтения протокола это «класс, который ловит ревью» → чинить ревью,
  а не добавлять стадию; резерв #267 ровно это и делает.

  Арифметика, которую триггеру придётся побить: у TASK-101 ревью стоило $2.09 —
  дороже любого другого вызова задачи; у TASK-104 из $9.55 на реализацию
  **$3.59 не купили ничего** (это #266), а на ревью не осталось. Автоматический
  refactor был бы четвёртой платной стадией там, где не оплачена третья.

  **Третий прогон (TASK-105) закрыл счётчик и подтвердил ту же картину:**
  post-GREEN долга в коде нет, а ревью снова не купили — `Task budget reached
  before the review call ($4.16 >= $4.00)`. Два из трёх evidence-прогонов
  влиты вообще без машинного ревью. Измерена и причина, по которой резерв не
  спас: он отнимает деньги у того, что видит **калитка** GREEN, но калитка
  отказывает только *следующему* вызову — RED кончился на $2.6944, чуть ниже
  потолка $2.75, GREEN стартовал и дошёл до $4.1643. Перерасход $0.16,
  ограничен одним вызовом ровно как задокументировано, — а один вызов стоил
  $1.47. **Резерв меньше одного типичного вызова ревью не защищает.** Это не
  дефект #267 (обещание сдержано), а граница, которую стоит записать.

  Рекомендация в #285 (решает владелец): закрыть как not planned, живой
  вопрос — оплата ревью, а не четвёртая стадия.
- 🚫 **Авторизация $4.00 отменена — цели нет.** Все четыре задачи m2 в статусе
  success; записанный расход домена **$26.95**, потолок в силе $28.00
  (авторизация #9). Цифра «минимум $4.35» из прежней формулировки — устаревший
  snapshot по четырём разрозненным state-файлам, смешивать её с текущей базой
  нельзя. Новые абсолютные лимиты считаются только после выбора третьей
  реальной задачи и свежего `costs --json`.
- 🐛 **#289 найден при подготовке той авторизации:** `budget authorize` с обеими
  осями и одним `--after` **применяется наполовину** — task-потолок пишется,
  run отказывается как stale, команда выходит 1 и о записанной половине молчит.
  До исправления — только две отдельные авторизации, каждая со своим `--after`.
- 🚢 **2.33.1 подготовлен** — только наблюдаемость: промпт каждого платного
  вызова записывается (#282 и его follow-up), поверхность не двигалась.
  Порядок владельца: нарезать **до** следующего платного прогона, потому что
  смысл прогона — evidence, которую потом можно прочитать.

  После публикации — репетиция PyPI-артефакта ровно в заданной
  последовательности: `--no-cache` → утверждение `spec-runner --version ==
  2.33.1` → только фейковый агент → RED-артефакт → промпт одиночного ревью →
  пер-ролевые артефакты → unknown-стоимость остаётся unknown → сбой логирования
  виден и не меняет вердикт. Только после этого — одна платная evidence-задача
  с новой дополнительной авторизацией $4.00 и резервом $1.25 на ревью.
- ✅ **#141 закрыт как функционально выполненный** (2026-08-14). Срезы 0–4a
  отгружены (#167, #171–#173, #181, #183, #188), поверх них прошёл весь
  hardening (#198, #207, #210, #213, #214, #220, #229–#232, #249, #252, #253,
  #260, #261, #263). Открытый статус создавал ложное впечатление, что
  `execution_mode: standard|tdd` не реализован.

  Единственный остаток — **3b, automatic REFACTORING** — вынесен в #285:
  deferred, триггер «три реальные завершённые TDD-задачи с повторяемым классом
  post-GREEN дефектов», шесть открытых вопросов намеренно без ответов до
  срабатывания триггера. Счётчик — в пункте «Operational-счётчик #285» выше;
  здесь он намеренно не повторяется, потому что число, записанное в двух
  местах, — это два числа. Работающий TDD lifecycle — закрытая
  возможность; автоматический REFACTORING — отдельная непроверенная гипотеза,
  а не хвост реализации.
- ✅ **v2.33.0 выпущен 2026-08-14 и отрепетирован.** Три подписанных решения:
  общий классификатор вердиктов ревью (#270), громкое сообщение о превышении
  потолка на обоих путях (#255 часть 1), доказательный тест в собственном файле
  (#252 вариант D). Поверхности измерены: тег на релизном коммите `ca495ef`,
  `publish.yml` success, PyPI 200 и `info.version` = 2.33.0, GitHub Release,
  красный `release-tag-guard` перезапущен зелёным, установка `--no-cache` с
  утверждением напечатанной версии.

  **Полная бесплатная репетиция опубликованного артефакта — PASS**, все три
  протокола проверены на установленном бинаре: red в существующий файл отказан
  (exit 1, замок не оставлен), тот же red в своём файле подтверждён, файл,
  который раннер не собирает, отказан с указанием формы адаптера, упоминание
  маркера ревью в прозе не читается как вердикт, резерв, названный только
  прозой, назван таковым, потолок отображается как авторизованный. Расход — 0.

  Два урока про сам стенд, оба мои: фейковый CLI печатал JSON, хотя команда не
  называется `claude` (значит результат читается как текст) — маркер оказался
  внутри строки; и сценарии шли в одном проекте, который после первого
  ожидаемого отказа встал по `max_consecutive_failures`, так что дальше
  измерялся собственный клин стенда. Теперь у каждого сценария свой проект.
- 🔭 **Репетиция нашла #282** — RED-промпт нигде не логируется, в отличие от
  промпта implementation-прохода. Это платный вызов, чей вывод становится
  селектором чекпойнта, и после прогона нет записи о том, что именно спросили;
  проверить опубликованный артефакт против его же промпта нечем. Не дефект
  релиза — пробел наблюдаемости, который старше него. **Закрыт** (#284): промпт
  пишется как `<TASK>-red-<timestamp>.log`; переиспользованный red не пишет
  ничего, а неудача записи предупреждает и не валит задачу. Промпт ревью
  по-прежнему не логируется — это отдельное решение, не следствие этого.
- ✅ **v2.32.1 выпущен 2026-08-14** — один патч, защита от уничтожения tracked
  state DB (#273), отдельно и сразу по решению владельца. Все поверхности
  измерены: тег на релизном коммите `bf9d6cd`, `publish.yml` success, PyPI 200
  на per-version и `info.version` = 2.32.1 (проектный эндпойнт отставал минуту,
  как и написано в ранбуке), GitHub Release создан, красный `release-tag-guard`
  перезапущен зелёным, артефакт установлен `--no-cache` и **напечатанная версия
  утверждена**.

  Репетиция против опубликованного артефакта прогнала ровно ту потерю: леджер
  ($9.00 с резервом на review) закоммичен в git, прогон **отказал с кодом 1**,
  БД осталась 90112 байт вместо нуля, авторизация цела. Затем напечатанная
  командой строка вставлена дословно — файл пережил `git rm --cached`, гард
  замолчал, следующий прогон завершил задачу, и леджер по-прежнему на месте.
- 📋 **Порядок работ подписан владельцем** (2026-08-14): 2.32.1 → #270 (общий
  классификатор вердиктов ревью) → #255 часть 1 (паритет `retry`/`run` и
  внятное сообщение о превышении) → #252 вариант D (доказательный тест в своём
  файле) → полная бесплатная репетиция опубликованного минора → и только потом
  отдельная новая авторизация платного пилота.
- ✅ **v2.32.0 выпущен 2026-08-14** — все четыре поверхности пройдены и
  измерены: тег на релизном коммите `68f9371`, `publish.yml` success, PyPI 200
  на per-version эндпойнте и `info.version` = 2.32.0, GitHub Release создан из
  секции CHANGELOG, красный `release-tag-guard` перезапущен зелёным, артефакт
  установлен `--refresh --reinstall --no-cache` и **напечатанная версия
  утверждена** (`spec-runner 2.32.0`). Индекс отстал и первая установка снова
  вернула `unsatisfiable` — пятый случай; репетиция не запускалась, пока версия
  не совпала.
- 🔬 **Репетиция опубликованного артефакта нашла #273** — не регрессию релиза, а
  давнюю тихую потерю данных: если state-БД отслеживается git, прогон обнуляет
  её до нуля байт и **выходит 0**. Вместе с ней исчезают леджер расходов,
  авторизации бюджета, red-чекпойнты и claims. Механизм измерен: staging
  снимает файл с учёта через `git rm --cached` (он держит runtime-состояние вне
  коммитов), а следующая операция с деревом пишет поверх живого SQLite пустоту.
  Репетиция дошла до этого случайно — `git add -A` до появления
  `spec/.gitignore`; ровно так до него дойдёт и пользователь.

  Содержимое — пять дверей TDD-цикла, все из одной платной фазы пилота,
  которая не могла закончиться: claim
  переживал задачу и морозил воркстрим (#260 → #265), отвергнутый red морил
  каждую следующую попытку (#261 → #268), `repair` отказывал ровно в том
  состоянии, ради которого сделан (#263 → #264), упоминание маркера в прозе
  побеждало настоящий вердикт (#266 → #269), и резерв на ревью нельзя было
  выразить (#267 → #271). Минор: две аддитивные операторские поверхности
  (`tdd release`, `budget authorize --reserve`) и две nullable-колонки;
  **вендоренные схемы не двигались**, поэтому синхронизация с
  spec-runner-vscode в этот раз не нужна. Одно намеренное изменение поведения
  выпущенной команды: `tdd repair` с проходящим тестом больше не пишет
  `not_red` lineage (выход по-прежнему 2).
- ⏸️ **Открыты три решения владельца, и все про смысл, а не про код.**
  **#252** — claim заморожен на red-коммите, а GREEN законно дописывает тесты в
  тот же файл, поэтому после `tdd resume` ни одно дерево с реализацией этот
  claim не удовлетворяет. Дизайн написан
  (`docs/superpowers/specs/2026-08-13-what-a-claim-protects-design.md`),
  рекомендация — «доказательный тест живёт в своём файле» сейчас и заморозка
  самого теста (а не файла) потом; «claim заканчивается на green» отклонён на
  тех же основаниях, на которых его отклонила подпись #232. **#255** — успешный
  путь потолок не проверяет вовсе; измерение поправило премису — гард стоит на
  всех трёх платных вызовах, а расходятся не успех и провал, а `retry` и `run`.
  **#270** — вердикты ревью читаются подстрокой так же, как читались терминальные
  маркеры до #266, и два пути ревью расходятся в приоритете, из-за чего вердикт
  зависит от того, включён ли бюджет. Кода ни по одному не пишу до подписи.
- 🧾 **Открытые issue: #285, #295 и #296.** Больше ничего; проверено
  `gh issue list`, а не памятью. #285 — automatic REFACTORING, счётчик закрыт
  3/3 с отрицательным результатом (пункт о нём выше). #295 и #296 — оба про
  логирование промптов, оба найдены артефактами прогона TASK-105. #289 закрыт
  в 2.33.2.

  Закрыто с перепроверкой на слитом дереве: #141, #252, #255, #260, #261, #263,
  #266, #267, #270, #273, #282 в этой волне; #218, #229, #230, #231, #232,
  #241, #245, #249, #253, #254, #256 раньше. У пяти формулировка находки
  поправлена в самом issue, потому что измерение назвало другую причину.

  «Issue открыт» и «фикс ждёт мержа» — разные состояния, и мешать их в одной
  строке значит писать статус, противоречащий сам себе (замечание Copilot к
  PR #286): пункт про #282 стоял рядом со словом «только». Открытые PR
  перечисляются отдельно, когда они есть.
- ✅ **v2.31.0 выпущен 2026-08-13** после того, как spec-runner-vscode#24 закрыт
  (их копия схемы байт-в-байт совпадает с нашей). Содержимое: леджер
  `pr_agent_calls` (#240, #218 этап 2), `budget authorize` (#243, #230 этап 2),
  `tdd resume` (#244, #232), ошибка git как instrument error (#246, #245),
  единый классификатор результата агента (#247, #241) и допустимость самого
  `resume` (#250, #249 — найден владельцем в течение часа после выхода #244,
  ровно в том состоянии, ради которого средство сделано).
- ✅ **Пост-релизная волна из шести issue разобрана** (#249, #253, #254, #256
  отгружены; #252 и #255 ждут подписи). Все шесть — из одного платного прогона
  по kapelle: `retry` мержил в master при `integration_pr`, потолок читали три
  места по-разному, FSM спрашивал предыдущую строку вместо доказательства.
- 💸 **Эта сессия дважды потратила деньги без разрешения — на тестах.**
  $0.545331 записаны в ledger (тест прогнал RED-фазу с дефолтным
  `claude_command`), плюс неоценённый вызов при проверке самого гарда. Оба раза
  инструмент вёл себя правильно, ошибка моя. Теперь `conftest._no_real_agent_calls`
  роняет такой тест вместо оплаты, `tests/test_harness_guards.py` закрепляет
  гард и **не может потратить сам**, а его граница привязана к списку
  поставляемых пресетов, а не ко второму рукописному списку.
- ✅ **v2.30.0 выпущен 2026-08-13** (тег на релизном коммите `9737765`, PyPI 200
  и `info.version` совпал, GitHub Release руками, красный `release-tag-guard`
  перезапущен зелёным, обе бесплатные репетиции против опубликованного артефакта
  зелёные). Тема: **инструмент знал, что произошло, и говорил оператору другое** —
  #226 (#218 этап 1), #227 (#220), #233 (#229), #234 (#231), #236 (#230 этап 1).
  Единственное внешне заметное изменение: TDD-прогон со сломанным
  replay-окружением теперь выходит `2`, а не `1`.
- ✅ **`--no-cache` внесён в ранбук** как обязательный флаг установки (#239):
  `--refresh --reinstall` в четвёртый раз оставил старый бинарь, и поймало это
  только утверждение напечатанной версии.
- ✅ **Оба дизайна подписаны 2026-08-13.** #230 этап 2: task-авторизация в области
  `(domain_id, namespace, task_id)`, run-авторизация — на весь домен и с
  `namespace = NULL` (иначе каждый namespace получит свой «глобальный» потолок;
  поправка владельца, закреплена CHECK-ом), отказ цитирует id/лимит/актора/время.
  #232: команда `tdd resume`, вариант (a). **Рекомендация дока по claims
  отменена:** восстанавливать надо checkpoint **и claims той же lineage одной
  транзакцией**, иначе цепочка «GREEN правит замороженный тест → repair
  вытесняет обоих → resume возвращает только red → merge» легально отмывает
  ровно то нарушение, ради которого byte-lock существует. **Оба реализованы**
  (#243, #244) ровно в подписанном виде, включая отменённую рекомендацию.
- 🧾 **#218 закрыт целиком** (#240), и выросший из него #241 — тоже (#247):
  `verify_comment` и `run_code_review` больше не читают один результат агента
  по-разному. Блокер релиза описан один раз, в шапке этого раздела.
...
- 🚫 **Новых платных прогонов не авторизовано.** Пункт исторический: тогда
  evidence было 0 из 3, ни одна попытка не доходила до post-GREEN, а «не меньше
  $4.35» было устаревшим snapshot'ом по четырём разрозненным state-файлам.
  Сейчас счётчик 3/3, канонический домен один (`c9051f20d1104a39`), записанный
  расход **$31.11** при потолке $30.95 — то есть следующая задача в этом домене
  начнётся с отказа, пока не будет новой авторизации абсолютной суммой поверх
  потраченного. Формулировкой «остаток бюджета» авторизовать по-прежнему
  нельзя (§5 дизайна `budget authorize`).
...
- 🩹 **#231 был потерей данных, и шире, чем в issue.** Вытирание дерева
  (`git checkout -- .` + `git clean -fd`) стоит в `pre_start_hook` **до всякого
  гейта** — воспроизведено на голом репозитории без TDD и claims. То есть так
  вёл себя любой старт задачи с git-автоматикой, а не только отказ по claim; и
  строка «refusing to implement» печаталась уже после мутации. Теперь всё, что
  есть в дереве, уходит в помеченный stash, а если stash не удался (включая
  нечитаемый `git status`) — задача не стартует. Восстановление доказано md5 по
  дереву до и после `git stash pop`.
- 🔬 **Каскад F-25 → F-28 пришёл с прогона на 2.28.3, не на 2.29.0** (проверено
  по времени: прогон 2026-08-12 13:09Z, #215 влит 2026-08-12 15:18Z, 2.29.0
  опубликован 2026-08-13 04:28Z). Три вклада в тот сбой уже закрыты в 2.29.0: ранний claims-check
  не дал бы платить за ревью обречённому кандидату, стоимость ревью теперь
  учитывается, guard проверяет бюджет перед каждым вызовом. Собственно провал
  задачи вызвал **claims-гейт** (GREEN-проход изменил замороженный
  `catalog_test.exs`: claimed `08d560c9`, found `3cfbf07f`), а не session limit —
  тот лишь оставил ревью без вердикта. Отсюда же следствие для #230: `$2.53` —
  пол, а не сумма (стоимость ревью тогда не писалась), и арифметику любого из
  трёх направлений имеет смысл пересчитать на текущем артефакте.
- 🔎 **Открыты из каскада:** #230 (этап 1 отгружен в #236; этап 2 — авторизованное
  расширение, дизайн написан и ждёт подписи. Возврат стоимости и отдельный
  infra-бюджет владелец решил **не вводить** — оставлены триггерными гипотезами
  до накопления корректно классифицированной статистики, которую как раз и даёт
  этап 1) и #232 (после `tdd repair` чекпойнт честно `not_red`, потому что работа
  сделана, а единственная дверь цикла — `red_authoring`: задача не может дойти
  до completed через контур). #232 — прямой вход в решение по #141.
- ✅ **#229 и #231 закрыты** (#233, #234), оба с перепроверкой на слитом дереве
  и с поправками к формулировке находки, записанными в самих issue: премиса
  F-25 («session limit съедает бюджет отказов как red-вердикт») не подтвердилась
  — задачу завалил claims-гейт, — а F-27 оказался шире заявленного.
...
- [x] PR-B пилота: caller-workflow гейта codex-review — влит #313 (`a35cdc3`, 2026-08-23) @owner:github:andrei-shtanakov @id:codex-review-caller
  ...
  двухшаговая дисциплина из шапки checksum.sh.
...
- [x] **post-review-plugin-edge** (inbox spec-runner#307, from disputatio) @owner:github:andrei-shtanakov @id:post-review-plugin-edge
      Точка `post_review` между ревью и финальным коммитом: записанное плагином
      в рабочее дерево подметает `stage_all_except_runtime` и уносит в коммит.
      Место — после вердикта ревью, записи `REFACTORING` и **успешных**
      пре-терминальных гейтов, непосредственно перед DONE-обновлением
      `tasks.md` и `commit_task_work`. Оценка стоимости оказалась
      пессимистичной: `plugins.py` **не менялся** — `run_plugin_hooks` резолвит
      точку по строке из манифеста, так что новая точка это call-site и ничего
      больше; три сайта сведены на общий `run_plugin_hooks_for`, потому что
      различаются только моментом срабатывания.

      Две вещи, которых в запросе не было. **(1)** Точка живёт только на
      успешном пути: заблокировавший гейт, отклоняющий HITL-вердикт и упавший
      таск до неё не доходят — экспорт evidence про остановленную попытку это
      ровно артефакт, читающийся как завершённая работа, то есть класс дефекта,
      ради которого гейты и существуют. **(2)** Блокирующий отказ отдаётся в
      resumable-форме гейта через `_commit_blocked_status`, а недописанное
      упавшим экспортёром остаётся **незакоммиченным** — вывод сломавшегося
      экспортёра не evidence.

      Замечание Copilot было одно и верное: проза утверждала безусловно, что
      флип `🔍 REVIEW` коммитится, — а `_commit_blocked_status` делает это
      только под `auto_commit`. Правлено в четырёх местах и запиннено
      характеризующим тестом, чтобы доки не разъехались молча.

      Вариант B (расширить `audit_log` событиями TDD-слоя) отклонён по
      измеренной причине: `spec/.executor-audit.jsonl` попадает под `.executor-*`
      из `spec/.gitignore`, который пишет сам `ensure_runtime_gitignore`, —
      `git add -A` его не стейджит, tracked evidence не выходит. Оговорка
      согласована с заявителем: под `wants_candidate` evidence ложится в
      финальный bookkeeping-коммит, требование уточнено до «в одной
      доставляемой истории/PR». Тесты:
      `tests/test_post_review_plugin_edge.py` (18) (PR #308)

**Принято 2026-09-01: #330** (inbox, from devtools) — потолок бюджета живёт в
state-DB, а `--spec-prefix` меняет файл базы (`config.py:397` →
`spec/.executor-<prefix>state.db`), поэтому `authorize` без префикса и прогон с
префиксом говорят о разных потолках, каждый правдиво и ни один — о том же числе.

- [x] **budget-authorize-state-db-namespace** (inbox spec-runner#330, from devtools) @owner:github:andrei-shtanakov @id:budget-authorize-state-db-namespace
      Инцидент kapelle/WS-kapelle-47 (2026-08-31): прогон отказал
      `budget_exceeded ($2.55 > $1.82)` — это run-потолок из `stop_cause`
      (`state.py:1865`), где `effective_limits` не нашёл авторизации в
      префиксной базе и вернул конфигный `budget_usd`; `budget authorize
      --run-limit 20` без префикса читал дефолтную базу и отвечал «уже
      авторизация #1 ($35.95)». Оба сообщения верны, каждое в своей базе, и ни
      одно не называет базу.
      Три части, каждая — отдельное утверждение:
      **(1)** сообщения `authorize` и budget-отказа прогона называют домен
      (путь state-DB либо неймспейс), о котором говорят — минимум приёмки;
      **(2)** `authorize` без `--spec-prefix` при существующих префиксных базах
      в `spec/` предупреждает, что базы не одна, — расхождение видно до, а не
      после платного прогона;
      **(3)** решение по контракту: докстринг `effective_limits`
      (`budget.py:143`) обещает run-потолку «принадлежит всему budget domain и
      не несёт неймспейса», но домен — это файл базы, а префикс делает второй
      файл; выровнять либо реализацию с обещанием, либо обещание с реализацией.
      Признак готовности из issue: оператор из текста ошибки понимает, что
      `authorize` нужен `--spec-prefix` (или почему нет).

      Сделано. Домен называется через **один** резолвер (`budget.domain_label`)
      во всех четырёх местах, где цитируется потолок: решение `authorize`, его
      CAS- и monotonic-отказы, `budget_exceeded` от прогона и отказ до-вызовного
      гейта — две спеллинга были бы ровно тем же дефектом, входом со стороны
      представления. Предупреждение (2) печатается только когда оператор домен
      **не назвал**: с `--spec-prefix`/`--change` выбор уже сделан вслух, и
      список соседей там шум. По (3) выбран вариант A — обещание выровнено с
      реализацией: разделение по префиксу не дефект, а то, ради чего префикс и
      берут (`--budget` ограничивает пожизненный расход одного state-файла, фазы
      считаются раздельно); неверна была фраза докстринга, читавшаяся как «один
      потолок на все префиксы». Побочно из того же разбора и отгружено раньше:
      дремота бюджета выровнена на всех сайтах (PR #332) — забытая авторизация
      связывала задачу в проекте, снявшем ключи бюджета. Стенд проверен на
...
- [ ] **runtime-state-artifact-export** (spec-runner#480) @owner:TBD @id:runtime-state-artifact-export @epic:eco.spec-toolchain
      ...
      **Блокер снят 2026-09-21: devtools#282 закрыт живой приёмкой.** Тег
      `@blocked_by` снят тем же изменением — устаревший блокер сам по себе
      находка (`cross-repo-waits.md`).

      Доставлен не тот механизм, которого мы просили, а лучший: вместо
      извлечения путей из прозы «Предмета» — **структурный носитель**
      `delivers` с `dt_contract_version: 2`, где DT объявляет свои
      результаты, а связь с чек-листом **объявляется**, не угадывается
      (`covered_by` для покрытого пункта, `restates` для повтора
      обязательства). Живой прогон на своём репо: 3 DT, 22 результата, 14 с
      `covered_by`; гейт — 0 ошибок; проекция довезла все 22 id.

      **Переавторено под `dt_contract_version: 2` (2026-09-22, correction-PR
      узла decomposition):** 72 результата `delivers` на 14 DT, `sources`
      только на разрешимые id (`design#Q-NN`, `requirements#FR/NFR`,
      `acceptance#AC`), `covered_by` там, где DT сам называет сценарий, один
      `restates` (DT-12 повторяет диапазон retention DT-01 — §3b.6). Гвард
      моста (`dt_contract_findings`, read-only у соседа): 0 ошибок. Сухой
      рендер `render_tasks_dt`: все 72 `DEL` доезжают; толщина чек-листов
      TASK-001 2→6, TASK-003 2→8, TASK-011/012 2→4 — ровно те задачи, на
      форме которых сгорели $7 TASK-001. Узел переведён в `draft`
      (подписанные байты изменились), tasks-спека станет `stale` каскадом.
      **Дальше — владелец:** `--approve-node decomposition`, затем
      `--supersede` tasks-спеки (v3, статус TASK-001 `done` сохраняется по
      §I11) и её одобрение; только после этого — платные прогоны. Правка
      BEH-28 (`tls: n/a`) в эту волну НЕ включена: она в behaviour-spec и
      каскадировала бы переодобрение четырёх узлов, а не одного — решать
      владельцу, включать ли её сейчас.

      **TASK-001 закрыта в спеке (PR #573, `1c6b80f`):** предмет DT-01 в
      master через PR #556; без флипа `run --all` заплатил бы за неё второй
      раз. Находка: у tdd-задачи, доведённой руками до master мимо хука,
      нет санкционированной двери для снятия claims (`release` требует DONE,
      `abandon` — ложь о законченной задаче); claims red-теста остаются
      активными, red-тест в master зелёный.

      Чем это было (сохранено как измеренная цена):
      ...
      **Решено владельцем 2026-09-22 (бандл PR #577, код — ветка
      `fix/480-beh-28-tls-tri-state`): BEH-28 переписан на «объявить
      свойство» — `tls` трёхзначно, `n/a` допустимо только когда
      применимость отрицает сам адаптер (`AdapterDeclaration`), сетевой
      адаптер обойти TLS строкой не может. Абзац ниже — история вопроса.**
      Открытый вопрос бандла, найденный реализацией (ревью TASK-001,
      круг 6): единственный поставленный адаптер нельзя объявить правдиво.
      Design § 1.2 говорит, что `LocalVolumeStore` объявляет `tls: n/a` —
      у пути на диске транспорта нет. BEH-28 требует от конфига `tls: true`,
      иначе отказ на загрузке. То есть оператор, использующий единственный
      существующий адаптер, обязан объявить про локальный том неправду,
      чтобы пройти гейт, написанный ради правдивости объявлений.

      Код приведён к § 1.2 (`StoreCapabilities.tls` трёхзначен: `True` /
      `False` / `None` = неприменимо), гейт НЕ тронут — его условие
      принадлежит BEH-28, а узел одобрен. Решать владельцу, и вариантов два:
      гейт принимает «неприменимо» как выполненное условие для адаптеров без
      транспорта, либо BEH-28 переписывается на «объявить свойство», а не
      «объявить его истинным». Первое дешевле, второе честнее. Правка любого
      из них — волна переодобрения цепочки, поэтому копить вместе с другими
      правками бандла.
...
**Дрейф `upstream_hashes` внутри бандла оставлен намеренно, и объём его —
свойство, а не перечень: после правок 2026-09-20 просрочены ВСЕ пины
бандла.** Байты сдвинулись у всех шести узлов (у четырёх — пересчётом
ссылок на `cli.py`), поэтому каждое ребро цепочки держит пре-правочный
блоб своего upstream-а; перечислять их поимённо смысла нет — список
разойдётся с деревом на первой же следующей правке, что и есть тот режим
отказа, о котором бандл пишет сам. Это не забытый хвост: по DEC-008 пин
ставится **только при переходе узла в `approved`** — обеими дверями,
...
- [x] **beh-09-close-call-scenario-unreachable** (spec-runner#480, из ревью PR #522) @owner:TBD @id:beh-09-close-call-scenario-unreachable
      ...
      **Закрыто 2026-09-20, после решения владельца о §3 (ниже).** Под
      широким прочтением конфигурации «C = `run` с open call и без
      acknowledged checkpoint-а» не существует вовсе: флип `in_progress`
      стоит до первого платного вызова, а publisher ожидается перед
      call-start. Поэтому C переписан на **`plan`** — платящую подкоманду
      из блокирующей половины, чей платный вызов предшествует любой её
      continuation-relevant mutation (задачи дописываются в `tasks.md`
      после вызова; проверено по коду: до `subprocess.run`
      `cli_plan.py:660` не пишется ни state DB, ни `tasks.md`). Первое
      условие ключа при этом ВЫПОЛНЯЕТСЯ, то есть конфигурация проверяет
      ключ, а не обходит его. Рядом предъявлен обратный ответ: тот же
      сценарий с C = `run` обязан ОТКАЗАТЬ шагом 5, и применённый там
      snapshot — красный тест. Правка в BEH-09 и AC-08.

      Предложенный вариант «`run`, остановившийся на pre-run отказе до
      status flip» не годится по собственным условиям: такой прогон не
      делает платного вызова вовсе, значит open call-а не оставляет и
      прогоном C быть не может.

      **Решение владельца, которого не хватает — расхождение §3 и §3.1 по
      ...
      публикации не имеет. Замерено по дереву: `commit_status_flip` зовут
      три сайта — blocked-путь (`hooks.py:724`),
      `commit_status_flip_quietly` с терминальных отказов
      (`bookkeeping.py:375`) и `recover_interrupted_flip`
      (`bookkeeping.py:347`). Флип `in_progress` не проходит ни через
      один из них в своём прогоне: третий коммитит его уже следующим
      прогоном, под другим `run_id`. То есть §3.1 описывал лишь часть
      доставленных путей.

      **Решено: широкое прочтение.** Continuation-relevant определяется
      свойством «без этой записи следующий шаг повторяет работу или
      теряет ограничение», и флип `in_progress` им обладает —
      возобновление его читает. Перечень §3.1 требование §3 не сужает:
      `commit_status_flip` — половина КОММИТА флипа, а не граница его
      публикации. §3.1 и BEH-13 приведены в соответствие:
      `task.update_task_status`, вызванный harness-ом, — точка публикации
      наравне с `record_*`.

      **Следствие для реализации (DT-02/DT-03):** seam обязан вызываться
      из **сайта записи** флипа в `tasks.md`, а не из функции
      `task.update_task_status` — у неё есть вызывающие вне harness-а
      (`task_commands.py:136/157/179`, `github_sync.py:166`), чьи
      подкоманды в перечне платящих FR-01 не состоят и run-start не
      получают, так что публикация оттуда шла бы под несуществующим
      `run_id`. Сайты перечислены в §3.1. Наблюдаемое — checkpoint,
      доставленный ДО первого call-start задачи (BEH-13).

      **Открытый хвост того же решения — записи спек-артефактов
      `plan --full`/`--gated`.** Свойство, которым обосновано широкое
      прочтение, распространяется и на них: `cli_plan.py:694` пишет
      артефакт стадии сразу после её платного вызова, и без этой записи
      следующий шаг повторил бы оплаченную работу. Считать ли их
      continuation-relevant — решением 2026-09-20 не закрыто; §3 их не
      перечисляет. BEH-09/AC-08 от ответа не зависят: их C пинован на
      **интерактивную** форму `plan`, которая до подтверждения оператором
      не пишет ничего.
...
PR-ом, что этот пункт; реализация придёт с DT-02/DT-03/DT-05. Из #527
третья просьба (transactional pending-outbox) вынесена **отдельной
...
- [ ] **bundle-480-narrow-checkpoint-delivery-window** (spec-runner#527, from devtools) @owner:github:andrei-shtanakov @id:bundle-480-narrow-checkpoint-delivery-window @epic:eco.spec-toolchain
      ...
      closure (Q-05, точка (б)) объявлен гейтом — подкоманда без платного
      вызова не завершается успешно, пока её mutation-checkpoint не
      acknowledged, таймаут → exit 2, а не ложный успех (строка,
      напечатанная handler-ом раньше отказа, успехом не считается, и
      диспетчер говорит в stderr, что решение записано локально, но не
      доставлено). Ожидание у подкоманды без платного вызова одно и на её выходе
      (у платящей — плюс по одному перед каждым вызовом; уточнено
      2026-09-20 правилом Q-05): точку «сразу
      после mutation» дизайн рассмотрел и отверг — её отказ рвал бы
      многошаговый `tdd abandon`/`repair` между записями, оставляя решение
      применённым наполовину и без audit-строки, и стоил бы одного ожидания
      на каждую mutation (у `repair` их четыре).
      Окно этим сужено до хвоста handler-а, но **не устранено**: `kill -9`
      внутри него по-прежнему теряет правку, и остаток остаётся принятым
      пробелом FR-05, как и был.
      Третье, что просила заявка, — «полезное усиление, transactional
      pending-outbox в SQLite» — **вынесено в spec-runner#528** (решение
      владельца 2026-09-18 по объёму): одиннадцать кругов терминального
      ревью дали двенадцать блокирующих находок, семь из них принадлежали
      усилению, а не заявкам, и сходимости за последние четыре круга не
      было. Всё установленное о нём записано в #528. Главное из
      установленного — что на пути «crash → restore» доставка обязана
      случиться раньше, чем `restore` читает store, — **перестало быть
      вынесенным** 2026-09-20: это граница ожидания, и она названа
      правилом Q-05 (см. следующий пункт). За #528 остались сами
      обязательства и носитель, которым они доезжают до очереди
      `restore`.
      Наблюдаемое — BEH-48/AC-46 (AC-46 держит статус experimental отдельной
      строкой порога), владелец знака — DT-05. **Не закрыт:** решение
      зафиксировано в бандле этим PR-ом, код — DT-03 (гейт и его отказный
      режим) и DT-05 (сайты mutation неплатящих подкоманд).

      **Развилка #528 снята 2026-09-20 (решение владельца): правило Q-05
      переписано свойством.** Было — перечень двух точек drain (перед
      call-start, перед closure); стало — одно правило с тремя сайтами:
      пока у процесса есть недоставленные обязательства, он не тратит (а),
      не объявляет завершение (б) и не принимает по store решение, которое
      их доставка развернула бы в сторону отказа (в). Различитель —
      направление ошибки устаревшего store (fail-open против fail-closed),
      а НЕ «выводимо локально или нет»: по store решает и ветка (1) Q-12.
      Сайт (в) — забор на ЧТЕНИЕ перед первой из семи проверок `restore`
      и перед выбором материала (проверка (2) берёт digests последнего
      acknowledged checkpoint-а): дренаж после любого из этих чтений
      бесполезен, и вердикт, и выбранный snapshot уже посчитаны.

      Ключевое, что это разблокировало: записанный отказ «третьей точки
      нет» относился к точке **«сразу после mutation»** — все его
      аргументы про `after_mutation`, про ожидание на каждой mutation и
      про `Refusal` из середины многошагового handler-а. К забору на
      границе invocation-а ни один из них не применим, и отказ был
      засчитан не тому объекту. Область (в) ограничена: выбор задачи
      процедурой Q-12 ею не накрыт — недоставленная mutation уже в
      локальной DB, и store там не единственный источник (это записано в
      #528 п. 4 и теперь предъявлено обратным знаком в BEH-48).

      Правки: FR-03 в requirements, §Q-05 (формулировка + различитель по
      направлению ошибки + область), §3.1, §3.5, §7.2 (забор перед первой
      проверкой и перечень instrument-исходов), карта модулей, DT-03 в
      decomposition.

      **Наблюдаемого забора в этом бандле НЕТ, и это записано прямо**
      (решение владельца 2026-09-20, вариант A). Вход для знака —
      унаследованное недоставленное обязательство — бандлом не
      производится: obligation-строки и их подъём в очередь publisher-а
      при первом открытии DB суть механизм #528, а очередь процессная и у
      нового процесса `restore` пуста по построению. Знак «ack раньше
      первого обращения к store» на пустой очереди красил бы неизвестное
      зелёным, знак «с отклоняющим двойником exit 2» недостижим. Поэтому
      BEH-48/AC-46 несут не знаки, а **объявление зависимости**, а сам
      вызов drain и его тест приезжают с #528, который владеет и
      механизмом, и наблюдаемым. Место забора при этом — решение ЭТОГО
      бандла: без него механизм цели не достигает.

      **Что из #528 остаётся:** сами obligation-строки в транзакции
      mutation, их подъём в очередь publisher-а при первом за процесс
      открытии DB, правило для обязательства без локальных байтов
      (пересъёмка + `absorbed`) и снятие унаследованных обязательств при
      `restore`.

      Пины `upstream_hashes` этой правкой не пересчитаны, и это то же
      объявление, что выше по этому пункту: репин драфта руками запрещён
      (DEC-008 — пин ставит только переход узла в `approved`, ручная
      перепиновка черновика есть дефект #426). После правок волны
      просрочены ВСЕ пины бандла; встанут они одним approve цепочки.
...
- [ ] **harness-guard-companions** — четыре сопутствующих пункта из #137, @owner:github:andrei-shtanakov @id:harness-guard-companions @epic:eco.spec-toolchain
      ...
      2. **Текст гейта предлагает себя обойти** — «...or, if the change is
         ...
         буквально подсказываем, как снять барьер. **Сделано 2026-09-23:**
         подсказка ушла из `error` (= retry-промпт) в progress-строку
         оператора; регресс —
         `test_harness_guard.py::…::test_the_agent_is_not_told_how_to_exempt_itself`.
...
### ✅ Закрыто #234: `pre_start` затирал незакоммиченную работу (заведено 2026-06-11, исправлено 2026-08-13)
...
(hooks.py ~108-117) затирает незакоммиченную работу предыдущей задачи.
- [x] Теперь всё, что есть в дереве, уходит в помеченный stash до уборки; если stash @owner:github:andrei-shtanakov @id:no-commit-work-loss
      не удался (включая нечитаемый `git status`) — задача не стартует (#231 → #234).

**Чем этот пункт поучителен.** Он пролежал два месяца с пометкой «низкий приоритет
(ниша `--no-commit`)», потому что область была прочитана слишком узко: условием
записали `auto_commit=False`, хотя уборка уничтожает **любую** незакоммиченную
работу. При `auto_commit=True` это почти никогда не видно — харнесс сам коммитит
свою работу, — но ровно «почти» и сработало в пилоте: агент ревью умер на session
limit, его правки остались незакоммиченными, и следующий `retry` их стёр (#231).
Дефект был найден заново боевым прогоном как потеря данных, хотя лежал в плане
описанным. Мораль для триажа: «редко проявляется» — не то же самое, что «узкая
ниша», и цена срабатывания в оценку не входила.
