...
## Текущее состояние
- 🚢 **v2.32.0 подготовлен: bump и секция CHANGELOG сделаны.** Осталась
  публикация по ранбуку — тег на релизном коммите → `publish.yml` → проверка
  PyPI обоими способами → GitHub Release руками → перезапуск красного
  `release-tag-guard` → установка артефакта с `--no-cache` и утверждением
  напечатанной версии. Этот пункт станет «выпущен» тогда, когда всё это
  измерено, а не когда PR влит.
...
- 🧾 **Все issue закрыты, кроме #141, #252, #255 и #270.** #218, #229, #230, #231,
