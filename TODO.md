...
- [x] **beh-09-close-call-scenario-unreachable** (spec-runner#480, из ревью PR #522) @owner:TBD @id:beh-09-close-call-scenario-unreachable
      ...
      **Решение владельца, которого не хватает — расхождение §3 и §3.1 по
      ...
      публикации не имеет. Замерено по дереву: `commit_status_flip`
      зовётся только на blocked-пути (`hooks.py:724`), а
      `commit_status_flip_quietly` — на терминальных отказах
      (`execution.py:1392`, `:1438`); флип `in_progress` не проходит ни
      через один из них. То есть §3.1 описывал лишь часть доставленных
      путей.
