...
## Текущее состояние
- 🚢 **2.32.1 подготовлен: bump и секция CHANGELOG сделаны** — один патч,
  защита от уничтожения tracked state DB (#273). Публикация по ранбуку; пункт
  станет «выпущен», когда артефакт установлен и его версия утверждена.
  Владелец решил выпускать отдельно и сейчас: data-loss гард не должен ждать
  более крупных изменений.
