...
## Текущее состояние
- 🚀 **`[Unreleased]` не пуст: четыре PR ждут релиза (2026-08-12).** #215 (#214 —
  frozen-files блок во всех агентских промптах + ранний claims-check перед
  ревью), #216 и #217 (#213 обеими половинами — учёт стоимости ревью и
  pre-call budget guard), #221 (дефект guard, найденный бесплатной репетицией).
  Тема выпуска: **инструмент перестал тратить деньги вслепую.** Публичная
  поверхность меняется один раз — необязательный `unmeasured_calls` в
  `costs --json` (`schemas/costs.schema.json`), — поэтому **2.29.0**, минор.
  Схема вендорится в spec-runner-vscode с `additionalProperties: false`, им
  заведён inbox-issue (их #22) на обновление копии.
  Релиз **не начат**: ждёт решения владельца.
...
- 🔎 **Открытые решения после этого круга:** #218 (вызовы `review-pr` не
  попадают в ledger — вопрос ключа: они принадлежат комментарию в PR, а не
  задаче; заодно собственный cost-лимит `review-pr` считает половину своих
  вызовов), #219 (успешная влитая задача помечается BLOCKED, если бюджет
  кончился после неё — и `blocked → todo` даёт переисполнение уже влитой
  работы), #220 (RED-фаза линтует дефолтным `uv run ruff check .`, поэтому на
  проекте без объявленного линтера TDD не работает вовсе, а `unverifiable`
  превращает это в платные повторы).
