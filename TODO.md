...
- 🚀 **`[Unreleased]`: четыре PR после 2.29.0.** #226 (#218 этап 1 — собственный
  лимит `review-pr` считает оба вида платных вызовов и проверяется **перед**
  каждым; `verify_comment` наконец знает свою стоимость; неположительный
  `max_cost_usd` — `0` или отрицательный — выключает лимит), #227 (#220 —
  pre-freeze lint запускает только **объявленный** линтер, плюс отказ RED
  больше не теряет причину), #233 (#229 — session limit провайдера распознаётся
  как retryable infrastructure с временем сброса, подпись отказа называет
  причину, брошенные некоммитнутые правки попадают в причину блокировки),
  #234 (#231 — старт задачи больше не уничтожает несохранённую работу),
  #236 (#230 этап 1 — отказ классифицируется типом, а не формулировкой).
  Схем и флагов никто не добавляет, но **#236 меняет внешний exit-код**: TDD-прогон
  со сломанным replay-окружением теперь выходит `2` (сломан прибор) там, где
  выходил `1` (работа плохая). Это заявленная поверхность `run_exit_code`, и
  RED-площадка впервые способна её соблюсти; в release notes сказать явно.
- 📐 **Design-first по #230 этап 2:** `docs/superpowers/specs/2026-08-13-budget-authorization-design.md`
  — команда `budget authorize`, append-only запись (прежний и новый абсолютный
  лимит, recorded spend и число неоценённых вызовов на момент решения, actor,
  reason, CAS, отказ при живом прогоне, только повышение) и главная граница:
  бюджетный домен привязан к state DB, новый файл не наследует авторизацию,
  перенос требует явного opening balance. Ждёт подписи владельца; кода нет.
