...
- 📋 **Третья evidence-задача выбрана владельцем: S4 needs-human hold path**
  (kapelle). Границы: fixture-агент создаёт предложение с критичным unresolved
  gap → evaluator переводит loop в `needs_human` → состояние и причинные
  артефакты видны через canonical View → следующий job не ставится → повторный
  reconcile ничего не дублирует → совпадение с golden oracle. **Не входит:**
  human resume (заблокирован producer-owned контрактом impresario#14), полная
  fault-injection матрица, LiveView, реальный LLM, несколько S4-сценариев.

  Порядок до денег: PR S3 `plan/product-s3-workers-e2e` → CI, Copilot, human
  review → мерж → одна S4 product-task с закоммиченной спекой → чистые
  master/spec и версия 2.33.2 → свежие `costs --json` и последние
  authorization id → две отдельные команды авторизации (task-ось для новой
  задачи ожидаемо $4.00 без `--after`; run-ось `total_cost + 4.00` с `--after`
  последней run-авторизации; резерв `review=1.25` на обеих) → **показать
  числа до исполнения**. Платных вызовов до этого нет.
- 📊 **Operational-счётчик #285 — 2/3, триггер НЕ сработал.** Две evidence-записи
...
- 🧾 **Открытые issue: #285 и #289.** Больше ничего; проверено `gh issue list`,
  а не памятью. #285 — automatic REFACTORING, deferred (счётчик — в пункте о нём
  выше). #289 — половинное применение `budget authorize` с обеими осями.
...
- 🚫 **Платные прогоны по-прежнему не разрешены.** Evidence для 3b остаётся
  **0 из 3**: ни одна попытка не дошла до post-GREEN — единственного вопроса,
  ради которого пилот существует. Расход по kapelle — **не меньше $4.35** плюс
  неоценённый вызов ревью, разложен по четырём state-файлам; доказуемого остатка
  нет, поэтому следующую попытку нельзя авторизовать формулировкой «остаток»
  (см. §5 дизайна `budget authorize`). Найдено и отгружено одиннадцать дефектов
  прибора — это не проверяемая гипотеза.
