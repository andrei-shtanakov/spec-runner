...
## Текущее состояние
- ✅ **v2.29.0 выпущен 2026-08-13** (тег на релизном коммите `fb2dd9e`, PyPI 200,
  `info.version` совпал, GitHub Release руками, красный `release-tag-guard`
  перезапущен зелёным, `uv tool install --refresh --reinstall` с утверждением
  напечатанной версии). Тема выпуска: **инструмент перестал тратить деньги
  вслепую** — #215 (#214), #216 и #217 (#213 обеими половинами), #223 (#219),
  #224 (форматирование вендоримой схемы). Публичная поверхность изменилась один
  раз: необязательный `unmeasured_calls` в `costs --json`; копия схемы в
  spec-runner-vscode обновлена (их #22 → #23) и байт-в-байт совпадает с нашей.
  **Обе бесплатные репетиции прошли против опубликованного артефакта**, включая
  smoke `costs --json` против обеих схем.
- 🚫 **Платные прогоны по-прежнему не разрешены.** Условие владельца — отдельное
  решение о бюджете на попытку №4. Evidence для 3b остаётся **0 из 3**: три
  платных прогона стоили не меньше $3.71 при разрешённых $3.00 и ни один не
  дошёл до post-GREEN. Семь найденных дефектов прибора — не проверяемая гипотеза.
- ✅ **Закрыты как отгруженные (перепроверено по коду 2026-08-12):** #198, #207,
  #210 — висели открытыми, хотя их фиксы вышли в 2.27.1…2.28.3; #213 и #214
  закрыты мержем. Открытый issue с выпущенным фиксом читается как невыполненная
  работа, поэтому закрыты с указанием, что именно проверялось в дереве.
- 🚀 **`[Unreleased]` снова не пуст: два PR после 2.29.0.** #226 (#218 этап 1 —
  собственный лимит `review-pr` считает оба вида платных вызовов и проверяется
  **перед** каждым; `verify_comment` наконец знает свою стоимость; неположительный
  `max_cost_usd` — `0` или отрицательный — выключает лимит) и #227 (#220 — pre-freeze lint запускает только **объявленный**
  линтер, плюс отказ RED больше не теряет причину). Плюс #229 (F-25 — session
  limit провайдера распознаётся как retryable infrastructure, подпись отказа
  называет причину, брошенные некоммитнутые правки попадают в причину блокировки).
  Публичной поверхности не трогают: ни новых ключей схем, ни флагов; изменилось
  только поведение необъявленного дефолта. Кандидат — патч/минор по составу на
  момент выпуска.
- 🔬 **F-25/F-26 пришли из прогона на 2.28.3, не на 2.29.0** (проверено по
  времени: прогон 2026-08-12 13:09Z, #215 влит в 15:18Z, 2.29.0 опубликован
  08-13 04:28Z). Три из вкладов в тот сбой уже закрыты в 2.29.0: ранний
  claims-check не дал бы платить за ревью обречённому кандидату, стоимость
  ревью теперь учитывается, guard проверяет бюджет перед каждым вызовом.
  Собственно провал задачи вызвал **claims-гейт** (GREEN-проход изменил
  замороженный `catalog_test.exs`: claimed `08d560c9`, found `3cfbf07f`), а не
  session limit — тот лишь оставил ревью без вердикта.
- ✅ **#220 закрыт** (#227). Четвёртая граница владельца — «детерминированный
  отказ линтера не ретраить платно» — **измерена и уже выполнялась**: отказ линта
  не пишет checkpoint, гейт отвечает «нет подтверждённого red» (`UNSATISFIED`,
  не `INSTRUMENT_ERROR`) → `HOOK_FAILURE` → fatal, один платный вызов. Посылка
  issue про платные повторы не подтвердилась; свойство закреплено тестом, чтобы
  перестать быть случайным. Реально сломана была диагностика — гейт цитировал
  своё общее «no confirmed red» и терял причину; теперь обе половины.
- 🔎 **#218 остаётся открытым на этап 2:** отдельная таблица `pr_agent_calls`
  (не nullable `task_id` в `agent_calls`). До неё сессия `review-pr` невидима
  для `spec-runner costs`: её расход видит только собственный лимит петли и
  только внутри одного вызова.
  #219 закрыт мержем #223 и вышел в 2.29.0.
- ✅ **v2.28.3 выпущен 2026-08-12** — последний в серии из шести релизов того
  же дня (2.27.0 → 2.28.3), закрывших #182, #192 и весь трек #198 (адаптеры
  раннеров, среда реплея, типизированный selector). Все четыре поверхности
  проверялись каждый раз, включая установку опубликованного артефакта с
  `--refresh --reinstall` и утверждение напечатанной версии (`uv tool install`
  трижды молча оставлял старый бинарь — гард поймал каждый раз).
- ✅ **v2.24.0 выпущен** (запись «ждёт мержа и тега» устарела). Тема выпуска:
  приборы, которые не умели сообщить об отказе. `preflight --json` (read-only, #142a),
  честный вердикт стадии review (#138, `not_run`/`error` вместо тихого `passed`),
  резолв шаблонов промптов от `project_root` вместо CWD (#153), запись
  сгенерированной стадии только после валидации + ограниченный ремонт (#160).
  **Minor:** одно видимое изменение поведения — `plan --gated` на стойко
  невалидной генерации даёт ненулевой код и не оставляет файла; формат
  `--json-result` и схема state DB не тронуты, `ReviewVerdict` расширен
  аддитивно (схема обновлена в том же PR).
  После мержа: тег `v2.24.0` → `publish.yml` → **GitHub Release создать руками**.
- ✅ **v2.23.0 выпущен 2026-08-10** (PyPI + GitHub Release, тег на `a9e395e`):
  класс ложно-зелёного выхода, harness guard, строгость мета-строки, scoped
  tests, `TASK_BLOCKED`, authoring contract.
...
- [ ] **tdd-lifecycle-design** — #141 принят как **дизайн-трек**, не minor-релиз: @owner:github:andrei-shtanakov @id:tdd-lifecycle-design
      ...
      **Slice 2 отгружен** (#181): байт-замок на заклеймленные файлы.
      **Slice 3 отгружен** (#183): операторские remedy `abandon`/`repair` с CAS.
      **Slice 4a отгружен** (#188): lifecycle как записанная машина состояний,
      `REFACTORING` материализован с исходом `skipped` — агент не запускается.
      **Состояние на 2026-08-12 (решение владельца):** slices 0–4a — **shipped**;
      **3b / автоматический REFACTORING — deferred**, не гипотеза к реализации,
      а гипотеза к проверке. **Триггер:** минимум три реальные TDD-задачи
      показывают повторяемый класс post-GREEN дефектов, который отдельный
      refactor-pass исправляет лучше, чем review или следующая обычная задача.
      Без такого evidence автоматический refactor — это дополнительный дорогой
      LLM-вызов, новая mutation-стадия и ещё одна recovery-поверхность без
      доказанной пользы. Протокол сбора evidence (шесть полей на прогон):
      `docs/superpowers/specs/2026-08-12-tdd-evidence-protocol.md`.
      До накопления evidence #141 считается **функционально завершённым**.
      **Сбор evidence начат и пока не дал ни одной записи (2026-08-12).**
      Три платных попытки на kapelle (provider fallback), суммарно **не менее
      $3.71** при разрешённых $3.00 — «не менее», потому что стоимость ревью
      тогда не записывалась вообще. Ни одна не дошла до post-GREEN, то есть до
      вопроса, ради которого пилот существует. Зато каждая нашла дефект
      инструмента, и все они отгружены: #198 + #207 + #210 (ExUnit-адаптеры и
      среда реплея), #214 (GREEN-промпт не сообщал о byte-lock), #213 обеими
      половинами (стоимость ревью не учитывалась нигде; кэп не ограничивал
      попытку). Плюс две **бесплатные** репетиции на одноразовом клоне —
      нарушающий GREEN и бюджетная, — которые прошли всю цепочку и нашли ещё
      три дефекта: остаточный (мой) в самом guard, а также #219 и #220 —
      все три с тех пор отгружены (#221, #223, #227).
      Записи: `docs/superpowers/specs/2026-08-12-tdd-run-provider-fallback*.md`
      и `.../2026-08-12-tdd-rehearsals.md`.
      **Что это значит для триггера:** счётчик «три реальные TDD-задачи»
      по-прежнему **0**. Инструмент прошёл через семь дефектов и стал заметно
      надёжнее, но ни одного наблюдения о post-GREEN долге не получено, и без
      них 3b остаётся deferred по тому же основанию, что и раньше.
      **Условие следующей платной попытки** (правило пилота, не выполнено
      автоматически мержем): прогон против **опубликованного** артефакта с
      утверждением напечатанной версии, а не против рабочего дерева.
