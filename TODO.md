...
- [ ] **bundle-480-narrow-checkpoint-delivery-window** (spec-runner#527, from devtools) @owner:github:andrei-shtanakov @id:bundle-480-narrow-checkpoint-delivery-window
      ...
      прогона ни один **доставляющий** прогон не дошёл ни до платного
      вызова, ни до своего завершения (read-only команды и `restore` не
      доставляют по построению, так что успешный `status` доставки не
      гарантирует; в пределе — каталога и машины больше нет).
