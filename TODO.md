...
- [x] **beh-09-close-call-scenario-unreachable** (spec-runner#480, из ревью PR #522) @owner:TBD @id:beh-09-close-call-scenario-unreachable
      ...
      **Следствие для реализации (DT-02/DT-03):** seam обязан вызываться
      из сайта записи флипа в `tasks.md`, а не только из
      `bookkeeping.commit_status_flip`; наблюдаемое — checkpoint,
      доставленный ДО первого call-start задачи (BEH-13). Операторские
      флипы (`spec-runner task start|done|block`, `task_commands.py`) под
      «harness-written» не подпадают — отдельный вопрос, этим решением не
      закрытый.
