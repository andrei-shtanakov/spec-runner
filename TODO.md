...
- [ ] **bundle-480-narrow-checkpoint-delivery-window** (spec-runner#527, from devtools) @owner:github:andrei-shtanakov @id:bundle-480-narrow-checkpoint-delivery-window
      ...
      было. Всё установленное о нём (включая главное: на пути
      «crash → restore» доставка обязана случиться раньше, чем `restore`
      читает store в своих проверках, а его единственная точка drain стоит
      позже) записано в #528.
