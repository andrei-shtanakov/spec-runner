...
## Текущее состояние
- 🚢 **2.33.0 подготовлен: bump и секция CHANGELOG сделаны.** Три подписанных
  владельцем решения: общий классификатор вердиктов ревью (#270), громкое
  сообщение о превышении потолка на обоих путях (#255 часть 1) и доказательный
  тест в собственном файле (#252 вариант D). Минор, потому что каждый из трёх
  протоколов стал строже и может отказать конфигурации, которая раньше
  принималась случайно. Публикация — по ранбуку; пункт станет «выпущен», когда
  артефакт установлен и его версия утверждена, после чего идёт полная
  бесплатная репетиция и только затем — отдельное решение о новой платной
  авторизации.
