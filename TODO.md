...
## Текущее состояние
- ⏳ **Не зарелижено: 2.22.0 + класс A + #137.** master (`9a2267a`) несёт три влитых
...
- 🔴 **v2.22.0 собран, но не тегнут — CI-гард падает на master с 2026-08-08.**
  ...
  v2.4.0/v2.10.0) честно красный на релизном коммите `de9a31c` (run 31271852827) — то есть
  прибор сработал, а сигнал никто не снял. Практическое следствие: `pip install spec-runner`
  даёт 2.21.0 без task-status integrity, и version-pin у потребителей нечестен **уже
  третий раз подряд**. Найдено при разборе замечания Copilot к PR #143 про сбитую сводку
  версий ниже.
...
Тесты: `tests/test_exit_contract.py` (17). Попутно выровнены два входа того же
...
- [ ] **release-v2.22.0-tag** — тегнуть и опубликовать v2.22.0; проверить, почему красный @owner:github:andrei-shtanakov @id:release-v2.22.0-tag
