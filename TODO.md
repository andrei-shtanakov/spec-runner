...
## Текущее состояние
- 🚦 **`[Unreleased]` собран и ждёт одного внешнего шага.** Пять вещей: леджер
  `pr_agent_calls` (#240, #218 этап 2), `budget authorize` (#243, #230 этап 2),
  `tdd resume` (#244, #232), ошибка git как instrument error (#246, #245) и
  единый классификатор результата агента (#247, #241). Выпуск — **после**
  синхронизации строгой вендоренной схемы в spec-runner-vscode (их #24, открыт,
  движения нет): `costs --json` получает опциональные ключи PR-леджера, и их
  копия пинует `additionalProperties: false`. Порядок владельца: сначала
  schema-sync, затем минор со всеми пятью — чтобы новые операторские пути не
  выходили рядом с уже известными семантическими дырами.
- 🧾 **Все issue закрыты, кроме #141.** #218, #229, #230, #231, #232, #241, #245
  закрыты с перепроверкой на слитом дереве; у трёх из них формулировка находки
  была поправлена в самом issue, потому что измерение назвало другую причину.
...
- 🧾 **#218 закрыт целиком** (#240). Из него вырос #241: `verify_comment` и
  `run_code_review` по-разному читают один и тот же результат агента —
  ненулевой выход с текстом там принимается как вердикт, здесь отбрасывается
  (#156). Нужен общий типизированный классификатор результата.
- 🚦 **Релиз с `pr_agent_calls` ждёт** синхронизации строгой схемы в
  spec-runner-vscode (их #24).
