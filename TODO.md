...
- 📐 **Design-first по #232:** `docs/superpowers/specs/2026-08-13-post-green-resume-design.md`
  — `tdd resume` возвращает подтверждённому red статус активного (гейт не
  трогаем: новых способов его удовлетворить не появляется) и отказ `tdd repair`
  после green. Ждёт подписи владельца; кода нет.
