...
- ⚠️ **`uv tool install --refresh --reinstall` в четвёртый раз оставил старый
  бинарь** (`unsatisfiable` из устаревшего индекса; до этого 2.27.1, 2.28.1,
  2.28.3). Поймало только утверждение напечатанной версии; пробило `--no-cache`.
  Предлагаю внести `--no-cache` в ранбук как обязательный флаг, а не как
  средство на случай сбоя: без него репетиция идёт по старому бинарю и выглядит
  как проверка релиза, которой не является.
- ✅ **Оба дизайна подписаны 2026-08-13.** #230 этап 2: task-авторизация в области
  ...
  ровно то нарушение, ради которого byte-lock существует. Порядок реализации:
  #230 этап 2, затем #232.
