...
| `tdd_runners.py` | ~250 | Per-runner RED adapters (#198 §2, build order 1): `Selector`/`SelectorRefusal` (frozen, normalised project-relative paths, stable refusal codes), orthogonal `RunOutcome` (what the run did) and `SelectionProof` (whether the requested test ran) — only `TESTS_FAILED` + `PROVEN` is a red; `TddRunnerAdapter` protocol (`parse_selector`/`validate_command`/`preflight` — reads the **checkpoint** tree/`prepare_replay` → `ReplayEnvironment`|`ReplayEnvironmentRefusal`/`build_command`→argv/`classify`/`prove_selected`); #207: ExUnit shares dependency *sources* via `MIX_DEPS_PATH` and isolates the build under `_build/.spec-runner-replay-<id>` (forced: a build outside the project gets no relative `priv` symlink), proves the lock with `mix deps`, fetches nothing, and cleans up on every path, `PytestAdapter`, `ExUnitAdapter` (canonical `path:line`; AST preflight via `definition_lines` — Elixir's own parser proves the line defines a test *before* `mix` runs, because `path:line` silently resolves backwards to the nearest test; classification keys on the run summary, so a compile error is not a red) + `ADAPTERS` registry, `infer_adapter` (inference only where it cannot be wrong). ExUnit lands in build order 3 |
