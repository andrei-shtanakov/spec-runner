...
| `hooks.py` | ~515 | Pre/post hook orchestration, plugin hook integration; `pre_start_hook` gates `uv sync` on `sync_deps` config flag; `_note_stranded_work` names uncommitted work in a blocked task's reason (#229); `_claims_intact_before_review` asks the claims gate before paying a reviewer (#214 — a candidate that already broke the lock cannot be merged whatever the review says); `_commit_blocked_status` commits the blocked task's `REVIEW` flip via `bookkeeping.py` and appends any failure to the block reason (#192) |
