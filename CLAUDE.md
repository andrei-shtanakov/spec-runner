...
| `config.py` | ~500 | ExecutorConfig, Persona, YAML loading, build_config; supports both `spec-runner.config.yaml` (v2.0) and `spec/executor.config.yaml` (legacy); `ExecutorLock` with PID diagnostics; `_detect_subdir_repo()` + subdir-project git-automation auto-default (OFF when project_root is a strict subdir of a larger git repo) |
