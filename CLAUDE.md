...
| `budget.py` | ~230 | The pre-call budget guard (#213): `check_before_call(config, state, task_id, provenance, pending_cost)` → `BudgetRefusal(kind, reason)` / `BudgetRefused`, `budget_is_active`. A **guard, not a hard cap** — a call's cost is known only after it returns, so the guarantee is “once recorded spend reaches the limit no new paid call starts; the overshoot is bounded by one call”. Checked before each of a TDD attempt's three paid calls (RED → GREEN → review); refusal kinds `task_budget`/`run_budget`/`unpriced`/`unreadable`. `effective_limits(config, state, task_id, provenance=None)` resolves the ceiling after authorizations, minus any **reserve** the call is not entitled to (#267); with no provenance it answers the ceiling as authorised, which is what the preflight and `costs` ask. `pending_cost` is this attempt's spend that `record_attempt` has not written yet (it runs after `post_done_hook`); `None` means unknown, which fails closed like any unprovable remainder
