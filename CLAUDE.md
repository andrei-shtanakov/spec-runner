...
| `budget_cmd.py` | ~230 | `spec-runner budget authorize` (#230 part 2): an operator raising an absolute ceiling, audited — mandatory reason, recorded actor, lock/agent refusal, CAS via `--after`, raises only. Flags are `--task-limit`/`--run-limit`, deliberately not the inherited `--task-budget`/`--budget` (those override the config for one invocation, and reusing the spelling would move the value the monotonic check compares against) |
| `budget.py` | ~185 | The pre-call budget guard (#213): `check_before_call(config, state, task_id, provenance, pending_cost)` → `BudgetRefusal(kind, reason)` / `BudgetRefused`, `budget_is_active`. A **guard, not a hard cap** — a call's cost is known only after it returns, so the guarantee is “once recorded spend reaches the limit no new paid call starts; the overshoot is bounded by one call”. Checked before each of a TDD attempt's three paid calls (RED → GREEN → review); refusal kinds `task_budget`/`run_budget`/`unpriced`/`unreadable`. `pending_cost` is this attempt's spend that `record_attempt` has not written yet (it runs after `post_done_hook`); `None` means unknown, which fails closed like any unprovable remainder
