...
| `prompts_log.py` | ~110 | One writer for every paid call's prompt (#282 + follow-up): `log_prompt(config, task_id, provenance, prompt)` → the file it wrote, `append_output` for the answer/return code/cost beside it (`unknown`, never `0.0`), `bound` (head **and** tail, since the frozen-files block is appended last), `provenance_slug` (whitelist, so a role name cannot name a file outside `logs_dir`). Records the prompt **as sent**; never argv, environment or command name; a failed write warns and never fails the task |
