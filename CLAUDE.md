...
| `budget_cmd.py` | ~230 | `spec-runner budget authorize` (#230 part 2): an operator raising an absolute ceiling, audited — mandatory reason, recorded actor, lock/agent refusal, CAS via `--after`, raises only. `--reserve <stage>=<usd>` (#267) withholds part of the ceiling from every call that is not that stage's — review is the last paid call of an attempt and so structurally the one the remainder starves. Flags are `--task-limit`/`--run-limit`, deliberately not the inherited `--task-budget`/`--budget` (those override the config for one invocation, and reusing the spelling would move the value the monotonic check compares against) |
