...
| `hooks.py` | ~515 | Pre/post hook orchestration, plugin hook integration; `pre_start_hook` gates `uv sync` on `sync_deps` config flag; `_commit_blocked_status` commits the blocked task's `REVIEW` flip via `bookkeeping.py` and appends any failure to the block reason (#192) |
