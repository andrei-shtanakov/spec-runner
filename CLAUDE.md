...
| `gates.py` | ~250 | Pre-terminal policy gates (#164): `GateStatus` (satisfied/unsatisfied/instrument_error), `GateResult`/`GateContext` (`config_hash` over `POLICY_KEYS` only), `GateRegistry` + module-level `REGISTRY`, `evaluate_gates` (per phase) and `evaluate_pre_terminal` (all registered phases). A gate never withholds the checkpoint commit — it is evaluated *against* that SHA and withholds merge/DONE. Only `instrument_error` is retried (`gate_recovery_attempts`). Dormant with an empty registry; consumers: #157, #141 |
