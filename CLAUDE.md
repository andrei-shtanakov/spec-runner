...
| `bookkeeping.py` | ~200 | The status flip a gate-blocked task leaves behind, committed (#192/F-8): `status_only_transition` (one changed line, the named task's meta line, status only — else None) + `commit_status_flip` (stages only `tasks.md`, message carries `Task-Status`/`Gate-Candidate`/`Gate-Verdict` trailers). Idempotent through the diff, not a marker, so a resumed task grows no chain of REVIEW commits; the commit is a child of the candidate, never the candidate itself |
