...
| `lifecycle.py` | ~165 | TDD lifecycle machine (#141 slice 4a): `TddPhase`, `advance()` / `current_phase()` / `has_reached()` (#249 — the high-water mark; "has this happened" must be asked of the history, because backwards is legal and every wedge retry re-enters `red_authoring`) over the append-only `tdd_phases` table, `ILLEGAL` (only READY/RED_AUTHORING → GREEN\*, the one transition the contract is about — backwards moves are legal because remedies and retries need them). `REFACTORING` is materialised with `detail="skipped"`; no refactor agent exists |
