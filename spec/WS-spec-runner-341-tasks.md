...
**Checklist:**
- [x] реализовать BEH-23: Стоимость сценария #341 измерена под pytest и сопоставлена с базовой точкой
- [x] рамка red-дизайна (решение владельца 2026-09-05, после двух непригодных red): RED выполняет живой in-memory замер сценария внутри самого теста и проверяет СВОЙСТВА замера
- [x] RED не проверяет существование артефактных файлов (статический файл фальшивит red; untracked путь нереплеируем в worktree)
- [x] артефакт записывается отдельным зелёным тестом в tracked workstreams/WS-spec-runner-341/measurements/
- [x] в артефакте сохраняются фактические elapsed_seconds и cost_usd, а не только константы базовой точки
- [x] проверка группы: tests/test_red_autofix_cost.py (kind: e2e) зелёные на BEH-23
