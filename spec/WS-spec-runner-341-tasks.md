...
- [ ] рамка red-дизайна (решение владельца 2026-09-05, после двух непригодных red): RED выполняет живой in-memory замер сценария внутри самого теста и проверяет СВОЙСТВА замера
- [ ] RED не проверяет существование артефактных файлов (статический файл фальшивит red; untracked путь нереплеируем в worktree)
- [ ] артефакт записывается отдельным зелёным тестом в tracked workstreams/WS-spec-runner-341/measurements/
- [ ] в артефакте сохраняются фактические elapsed_seconds и cost_usd, а не только константы базовой точки
