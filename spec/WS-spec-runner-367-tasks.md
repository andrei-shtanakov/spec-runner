...
- **Q-07 — повторная попытка verify-first задачи:** evidence переиспользуется
  по правилам, аналогичным `_reusable_checkpoint`, при ТОЧНОМ совпадении всех
  трёх осей: commit SHA кандидата, policy-config hash и состав объявленной
  группы (FR-10: тот же состав; FR-11: наследование через изменившийся вопрос
  недопустимо). Любое расхождение — свежий живой прогон, не деградация до
  частичного переиспользования.
- **Q-09 — где живёт отображение `(RunOutcome, SelectionProof,
  ExecutionProof)` → исход:** одной таблицей-константой в модуле verify-first
  (единственный источник); тест FR-16 перебирает все комбинации ИЗ этой
  таблицы, три места соглашения не заводятся.
- **Потолок времени прогона (BEH-14, NFR-02):** объявленное значение — на
  СЕЛЕКТОР: `REPLAY_TIMEOUT_SECONDS` (сегодня 900 с, прецедент named-константы
  из BEH-14); потолок группы = сумма по её селекторам (форма прогона —
  по-селекторная, FR-06). BEH-14 проверяется против этой константы как
  ОБЪЯВЛЕННОГО значения (импорт), не против числа, зашитого в тест.
