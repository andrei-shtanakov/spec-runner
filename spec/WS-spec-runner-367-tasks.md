...
- **Q-07 — повторная попытка verify-first задачи:** evidence переиспользуется
  ...
  недопустимо); ЧЕТВЁРТАЯ ось — зелёное направление стареет в опасную
  сторону (в отличие от красного чекпойнта), поэтому переиспользование
  зелёной evidence требует ещё и совпадения git tree-hash кандидата с
  tree-hash коммита evidence: любое изменение дерева (в том числе кодом
  самой задачи после зелёного прогона) — свежий живой прогон (minor
  круга 5). Расхождение по любой оси — свежий живой прогон, не
  деградация до частичного переиспользования.
- **Q-09 — где живёт отображение `(RunOutcome, SelectionProof,
  ...
  ПОСТ-прогонную тройку; пред-прогонные и инструментальные отказы
  (отсутствующая evidence, недостижимое окружение, недостижимый SHA,
  композитная test_command — перечень BEH-22) классифицируются явной
  веткой `instrument-error` ДО прогона, вне таблицы, и покрываются
  отдельными кейсами теста BEH-23 (minor круга 5 — «единственный
  источник» относится к классификации исходов прогона, не к
  пред-прогонным отказам).
- **Потолок времени прогона (BEH-14, NFR-02):** на селектор —
  ...
  первого прогона.
- **Порядок исполнения (major круга 5):** регистрация гейтов и аудит
  чтений режима (BEH-24, TASK-009) исполняются ДО задачи, доводящей
  green-only до DONE (BEH-20, TASK-008): red-гейт обязан быть спрошен и
  удовлетворён ссылкой на evidence, что невозможно до регистрации;
  обратный порядок оставлял бы окно, где verify-first проходит red-гейт
  тривиальным SATISFIED (mode != tdd) — ровно трактовка «третий режим =
  гарантий нет», запрещённая FR-17/Q-01. Рёбра Depends on переставлены:
  …TASK-007 → TASK-009 → TASK-008 → TASK-010….
...
**Checklist:**
- [ ] реализовать BEH-10: Неразрешимый адаптер и композитная `test_command` — отказ, а не догадка
- [ ] реализовать BEH-11: `green` требует трёх фактов по каждому объявленному селектору
- [ ] реализовать BEH-12: Прошедший прогон с недоказанным выбором зелёным не является
- [ ] реализовать BEH-13: Пустой и пропущенный выбор — не здоровье
- [ ] реализовать BEH-14: Потолок времени прогона объявлен и превышение — отказ, а не зависание
...
Реализовать сценарии BEH-24, BEH-25, BEH-28, BEH-29.
Source: workstreams/WS-spec-runner-367/spec/15-behaviour-spec.md#BEH-24 (—BEH-29)
**Depends on:** [TASK-007]
