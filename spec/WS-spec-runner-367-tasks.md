...
- **Q-07 — повторная попытка verify-first задачи:** evidence переиспользуется
  ...
  недопустимо). Расхождение по любой оси — свежий живой прогон, не
...
- **Q-09 — где живёт отображение `(RunOutcome, SelectionProof,
  ...
  тест, не молчаливый четвёртый исход.
- **Потолок времени прогона (BEH-14, NFR-02):** на селектор —
  `REPLAY_TIMEOUT_SECONDS` (сегодня 900 с, прецедент named-константы);
  потолок ГРУППЫ — отдельная объявленная величина, вычисляемая при старте
  прогона как `len(selectors) × REPLAY_TIMEOUT_SECONDS` и логируемая ДО
  первого прогона; исчерпание группового потолка — `instrument-error`
  (BEH-14 проверяется против этой объявленной величины, суммарный
  wall-clock ограничен сверху, не «объявлен по частям», NFR-02). Оба
  значения потребляются импортом/вычислением из констант, не числом,
  зашитым в тест.
